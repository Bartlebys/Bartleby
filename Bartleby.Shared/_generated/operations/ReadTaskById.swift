...
public class ReadTaskById : JObject{
    ...
    public static func execute(fromDataSpace spaceUID:String,
						taskId:String,
						sucessHandler success:(task:Task)->(),
						failureHandler failure:(context:JHTTPResponse)->()){
	
				    let baseURL=Bartleby.sharedInstance.getCollaborationURLForSpaceUID(spaceUID)
				    let pathURL=baseURL.URLByAppendingPathComponent("task/\(taskId)")
				    let dictionary:Dictionary<String, AnyObject>=[:]
				    let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"ReadTaskById" ,forMethod:"GET", and: pathURL)
				    ...
				    r.responseJSON{ response in
				        ...
				        let context = JHTTPResponse( code: 723293786,
				            caller: "ReadTaskById.execute",
				           ...
				           let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
				            ...
				            if let statusCode=response?.statusCode {
				                if 200...299 ~= statusCode {
									if let instance = Mapper <Task>().map(result.value){					    
									    success(task: instance)
									  }else{
									   let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
									        context: context,
