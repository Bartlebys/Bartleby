...
public class ReadTasksGroupById : JObject{
    ...
    public static func execute(fromDataSpace spaceUID:String,
						tasksGroupId:String,
						sucessHandler success:(tasksGroup:TasksGroup)->(),
						failureHandler failure:(context:JHTTPResponse)->()){
	
				    let baseURL=Bartleby.sharedInstance.getCollaborationURLForSpaceUID(spaceUID)
				    let pathURL=baseURL.URLByAppendingPathComponent("tasksGroup/\(tasksGroupId)")
				    let dictionary:Dictionary<String, AnyObject>=[:]
				    let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"ReadTasksGroupById" ,forMethod:"GET", and: pathURL)
				    ...
				    r.responseJSON{ response in
				        ...
				        let context = JHTTPResponse( code: 1950359274,
				            caller: "ReadTasksGroupById.execute",
				           ...
				           let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
				            ...
				            if let statusCode=response?.statusCode {
				                if 200...299 ~= statusCode {
									if let instance = Mapper <TasksGroup>().map(result.value){					    
									    success(tasksGroup: instance)
									  }else{
									   let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
									        context: context,
