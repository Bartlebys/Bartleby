...
@objc(UpdateGroups) public class UpdateGroups : JObject,JHTTPCommand{
    ...
    required public convenience init(){
        self.init([Group](), inDataSpace:Default.NO_UID)
    }
    ...
    required public init?(coder decoder: NSCoder) {
        ...
        self.lockAutoCommitObserver()
		self._groups=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Group.classForCoder()]), forKey: "_groups")! as! [Group]
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(self._groups,forKey:"_groups")
    ...
    */
    init (_ groups:[Group]=[Group](), inDataSpace spaceUID:String) {
        self._groups=groups
    ...
    - parameter spaceUID:     the space UID
    */
    static func commit(groups:[Group], inDataSpace spaceUID:String){
        let operationInstance=UpdateGroups(groups,inDataSpace:spaceUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:152111408, caller: "UpdateGroups.commit")
        if let document = Bartleby.sharedInstance.getDocumentByUID(self._spaceUID) {
                ...
                let stringIDS=PString.ltrim(self._groups.reduce("", combine: { $0+","+$1.UID }),characters:",")
                self._operation.summary="UpdateGroups(\(stringIDS))"
    ...
    public func push(sucessHandler success:(context:JHTTPResponse)->(),
        ...
        if let _ = Bartleby.sharedInstance.getDocumentByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                // This document is not available there is nothing to do.
                let context=Context(code:2968022629, caller: "UpdateGroups.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
...
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                r.responseJSON{ response in
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of groups",
                            comment: "update of groups failure description")
