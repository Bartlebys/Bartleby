...
public class CreateTasksGroups : JObject,JHTTPCommand{

    // Universal type support
    override public class func typeName() -> String {
        return "CreateTasksGroups"
    }
    ...
    override public func mapping(map: Map) {
        super.mapping(map)
		self._tasksGroups <- map["_tasksGroups"]
    ...
    required public init?(coder decoder: NSCoder) {
		...
		self._tasksGroups=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),TasksGroup.classForCoder()]), forKey: "_tasksGroups")! as! [TasksGroup]
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(self._tasksGroups,forKey:"_tasksGroups")
    ...
    - parameter tasksGroups: the tasksGroups concerned the operation
    ...
    */
    init (_ tasksGroups:[TasksGroup]=[TasksGroup](), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
        self._tasksGroups=tasksGroups
    ...
    - parameter tasksGroups: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(tasksGroups:[TasksGroup], inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateTasksGroups(tasksGroups,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:3922655916, caller: "CreateTasksGroups.commit")
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateTasksGroups.execute(self._tasksGroups,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._tasksGroups)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:2864989163, caller: "CreateTasksGroups.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(tasksGroups:[TasksGroup],
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                let pathURL=baseURL.URLByAppendingPathComponent("tasksGroups")
                var parameters=Dictionary<String, AnyObject>()
                var collection=[Dictionary<String, AnyObject>]()

                for tasksGroup in tasksGroups{
                    let serializedInstance=Mapper<TasksGroup>().toJSON(tasksGroup)
                    collection.append(serializedInstance)
                }
                parameters["tasksGroups"]=collection
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"CreateTasksGroups" ,forMethod:"POST", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 2294495900,
                        caller: "CreateTasksGroups.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of tasksGroups",
                            comment: "creation of tasksGroups failure description")
