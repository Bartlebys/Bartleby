        ...
        NSDirectoryEnumerator *dirEnum =[fileManager enumeratorAtURL:[NSURL fileURLWithPath:folderPath]
                                          includingPropertiesForKeys:keys
                                                        ...
                                                        errorHandler:^BOOL(NSURL *url, NSError *error) {
                                                            glog(@"ERROR when enumerating  %@ %@",url, [error localizedDescription]);
                                                            return YES;
                                                        }];
                ...
                @autoreleasepool {
                    ...
                    if(!self.recomputeHash && [fileManager fileExistsAtPath:hashfile] ){
                        ...
                        if(!crc32ReadingError){
                            ...
                            glog(@"ERROR when reading crc32 from %@ %@",filePath,[crc32ReadingError localizedDescription]);
                        }
                    }else{
                        if (dataBlock) {
...
- (void)saveHashMap:(nonnull HashMap *)hashMap toPath:(nonnull NSString *)path{
    ...
    if(cryptoError){
        glog(@"String encryption error: %@", [cryptoError description]);
    } else {
        ...
        [jsonCrypted writeToFile:path
                      atomically:YES
                        encoding:NSUTF8StringEncoding
                           error:&error];
        if(error){
            glog(@"ERROR when writing hashmap to %@ %@", [error description],path);
