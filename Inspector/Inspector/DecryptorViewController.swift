...
class DecryptorViewController: NSViewController, AsyncDocumentProvider, PasterDelegate {
    open override var nibName: NSNib.Name { return NSNib.Name("DecryptorViewController") }
                ...
                if let first = objects.first {
                    ...
                    if c.count >= 3 {
                        ...
                        do {
                            ...
                            decryptedString = try Bartleby.cryptoDelegate.decryptString(crypted, useKey: Bartleby.configuration.KEY)
                            if let d = self.decryptedString?.data(using: .utf8) {
                                ...
                                if let metadata = report.metadata {
                                    _document.metadata = metadata
                                }
                                _document.logs = report.logs
                                _document.metrics = report.metrics
                                for c in _consumers {
