...
class MetadataDetails: NSViewController, Editor, Identifiable, NSTabViewDelegate, DocumentDependent {
    @IBOutlet var infosItem: NSTabViewItem!
    @IBOutlet var userItem: NSTabViewItem!
    @IBOutlet var triggerAnalysisItem: NSTabViewItem!
    @IBOutlet var operationQuarantineItem: NSTabViewItem!
    ...
    override var representedObject: Any? {
        ...
        didSet {
            _metadata = representedObject as? EditorOf
        }
    }

    // MARK: NSTabViewDelegate

    public func tabView(_: NSTabView, willSelect tabViewItem: NSTabViewItem?) {
            ...
            if let documentReference = self.documentProvider?.getDocument() {
                if let identifier = tabViewItem.identifier as? String {
                    if identifier == "TriggersAnalysis" {
                        triggersDiagnosticTextView.string = documentReference.getTriggerBufferInformations()
                    }
                    if identifier == "OperationsQuarantine" {
                        operationsQuarantineTextView.string = documentReference.metadata.jsonOperationsQuarantine
                    }
                }
            }
        }
    ...
    @objc public func refreshMetadata(notification _: Notification) {
        if let documentReference = self.documentProvider?.getDocument() {
            triggersDiagnosticTextView.string = documentReference.getTriggerBufferInformations()
            operationsQuarantineTextView.string = documentReference.metadata.jsonOperationsQuarantine
        }
