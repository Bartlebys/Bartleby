...
@objc(TriggersCollectionController) public class TriggersCollectionController : BaseObject,IterableCollectibleCollection{
...
// MARK: Identifiable
    ...
    required public init?(coder decoder: NSCoder) {
		...
		items=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Trigger.classForCoder()]), forKey: "items")! as! [Trigger]
		_startObservingAllItems()
    ...
    private func _startObserving(item: Trigger) {
        if _observedUIDS.indexOf(item.UID) == nil {
			...
			item.addObserver(self, forKeyPath: "created", options: .Old, context: &KVOContext)
			item.addObserver(self, forKeyPath: "updated", options: .Old, context: &KVOContext)
			item.addObserver(self, forKeyPath: "deleted", options: .Old, context: &KVOContext)
        }
    }
    ...
    private func _stopObserving(item: Trigger) {
        if let idx=_observedUIDS.indexOf(item.UID)  {
			...
			item.removeObserver(self, forKeyPath: "created", context: &KVOContext)
			item.removeObserver(self, forKeyPath: "updated", context: &KVOContext)
			item.removeObserver(self, forKeyPath: "deleted", context: &KVOContext)
        }
    }
