...
struct Chunker {
    ...
    var embeddedInADocument: Bool { return (document != nil) }
    ...
    init(fileManager: FileManager,
        ...
        document = embeddedIn
        _cryptoHelper = CryptoHelper(salt: Bartleby.configuration.SHARED_SALT, keySize: keySize)
    }
    ...
    public func breakFolderIntoChunk(filesIn assembledFolderPath: String,
                                     ...
                                     chunkMaxSize _: Int = 10 * MB,
                                     compress: Bool = true,
                                     encrypt: Bool = true,
                                     externalId _: String = Default.NO_UID,
                                     excludeChunks: [Chunk] = [Chunk](),
                                     progression: @escaping ((Progression) -> Void),
                                     success: @escaping ([Chunk]) -> Void,
                                     failure: @escaping ([Chunk], String) -> Void) {
        Async.utility {
            ...
            if let folderURL = URL(string: assembledFolderPath) {
                ...
                var paths = [String]()
                let enumerator = fm.enumerator(at: folderURL, includingPropertiesForKeys: keys, options: options, errorHandler: { (_, _) -> Bool in
                    false
                })
            ...
            if let fileHandle = FileHandle(forReadingAtPath: path) {
                ...
                if !self.embeddedInADocument && self.mode == .digestAndProcessing {
                    if self.destroyChunksFolder {
                        _ = try? self._fileManager.removeItem(atPath: chunksfolderPath)
                    }

                    _ = try? self._fileManager.createDirectory(atPath: chunksfolderPath, withIntermediateDirectories: true, attributes: nil)
                }
                ...
                func __writeData(rank: Int, size: Int, data: Data, to chunksFolderPath: String, digest sha1: String, position: Int, relativePath: String) throws {
                    ...
                    if !self.embeddedInADocument && self.mode == .digestAndProcessing {
                        _ = try self._fileManager.createDirectory(atPath: bFolderPath, withIntermediateDirectories: true, attributes: nil)
                    }
    ...
    public func joinsChunks(chunks: [Chunk],
        ...
        Async.utility {
            ...
            for chunk in chunks {
                if !filePathToChunks.contains(where: { (k, _) -> Bool in
                    k == chunk.nodePath
                }) {
            ...
            for (_, v) in filePathToChunks {
                ...
                if nodeNature == .file {
                    let chunksPaths = v.map({ (chunk) -> String in
                        assemblyFolderPath + "/.blocks" + chunk.relativePath // chunk.absolutePath
                    })
    ...
    public func joinChunksToFile(from chunksPaths: [String],
                                 ...
                                 externalId: String = Default.NO_UID,
                                 progression: @escaping ((Progression) -> Void),
                                 success: @escaping (_ path: String) -> Void,
