...
struct DeltaAnalyzer {
    ...
    func deltaChunks(fromFileAt path: String,
                     ...
                     completed _: @escaping (_ preserve: [Chunk], _ delete: [Chunk]) -> Void,
                     failure: @escaping (_ message: String) -> Void) {
        // We use a Chunker is .digestOnly mode.
        // simulated mode can be 5 X times faster and do not consume Disk Room.
        let chunker = Chunker(fileManager: fileManager, mode: .digestOnly, embeddedIn: _document)
                               ...
                               progression: { _ in
                                   ...
                                   for chunk in chunks {
                                       if node.blocks.contains(where: { (block) -> Bool in
                                           chunk.sha1 == block.digest && chunk.rank == block.rank
                                       }) {
