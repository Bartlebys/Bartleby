...
public final class BSFS: TriggerHook {
    ...
    public required init(in document: BartlebyDocument) {
        _document = document
        _chunker = Chunker(fileManager: FileManager.default,
                           mode: .digestAndProcessing,
                           embeddedIn: document)
        _boxDelegate = document
    }
    ...
    public func saveState() -> Data {
        let state = [
            "toBeUploaded": self._toBeUploadedBlocksUIDS,
    ...
    public func restoreStateFrom(data: Data) throws {
        if let state = try? JSONSerialization.jsonObject(with: data) as? [String: [String]] {
            _toBeDownloadedBlocksUIDS = state?["toBeDownloaded"] ?? [String]()
            _toBeUploadedBlocksUIDS = state?["toBeUploaded"] ?? [String]()
        }
    }
    ...
    public var baseFolderPath: String {
        ...
        if _document.metadata.appGroup != "" {
    ...
    public func mount(boxUID: String,
                      progressed: @escaping (Progression) -> Void,
                      completed: @escaping (Completion) -> Void) {
        do {
            let box = try Bartleby.registredObjectByUID(boxUID) as Box
            _document.send(BoxStates.isMounting(box: box))
            ...
            let nodes = box.nodes
            for node in nodes {
                let isNotAssembled = !isAssembled(node)
            ...
            func __popNode() {
                if let node = concernedNodes.popLast() {
                    ...
                    do {
                        try _assemble(node: node, progressed: { progression in
                        ...
                        _document.log("\(error)", file: #file, function: #function, line: #line, category: Default.LOG_DEFAULT, decorative: false)
                    ...
                    _document.send(BoxStates.hasBeenMounted(box: box))
                }
            }
            // Call the first pop.
            __popNode()
            ...
            completed(Completion.failureStateFromError(error))
            _document.send(BoxStates.mountingHasFailed(boxUID: boxUID, message: "\(error)"))
        }
    }

    public func unMountAllBoxes() {
        _document.boxes.forEach { box in
    ...
    public func unMount(box: Box) {
        for node in box.nodes {
            ...
            let assembledPath = assemblyPath(for: node)
    ...
    public func assemblyPath(for node: Node) -> String {
        if let box = node.box {
            return assemblyPath(for: box) + node.relativePath
        }
    ...
    internal func _assemble(node: Node,
                            progressed: @escaping (Progression) -> Void,
                            completed: @escaping (Completion) -> Void) throws {
        do {
        ...
        if fileExistsAtPath {
            ...
            func __chunksHaveBeenCreated(chunks: [Chunk]) {
                ...
                for (nodeRelativePath, groupOfChunks) in groupedChunks {
                    // Create the new node.
                    // And add its blocks
                    let node = _document.newManagedModel() as Node
                    if firstNode == nil {
                        firstNode = node
                    }
                    node.quietChanges {
                    ...
                    if box.isMounted {
                        let nodeAssemblyPath = assemblyPath(for: node)
                        do {
                            try _fileManager.copyItem(atPath: reference.absolutePath, toPath: nodeAssemblyPath)
                        } catch {
                            _document.log("Original Copy has failed. \nReferencePath:\(reference.absolutePath) \nassemblyPath:\(nodeAssemblyPath)", category: Default.LOG_BSFS)
                        }
                    }
                }
                ...
                if deleteOriginal {
                    ...
                    do {
                        try _fileManager.removeItem(atPath: reference.absolutePath)
                    } catch {
                        _document.log("Deletion has failed. Path:\(reference.absolutePath)", file: Default.LOG_BSFS)
                    }
                }
    ...
    func replaceContent(of node: Node,
                        ...
                        deleteOriginal: Bool,
                        accessor _: NodeAccessor,
                        progressed: @escaping (Progression) -> Void,
                        completed: @escaping (Completion) -> Void) {
        if node.nature == .file {
            if node.authorized.contains(_document.currentUser.UID) ||
                ...
                if fileExistsAtPath {
                    if isDirectory.boolValue {
                        ...
                        if let box = node.box {
                            let analyzer = DeltaAnalyzer(embeddedIn: _document)

                            //////////////////////////////
                            // #1 We first compute the `deltaChunks` to determine if some Blocks can be preserved
                            //////////////////////////////

                            analyzer.deltaChunks(fromFileAt: path, to: node, using: _fileManager, completed: { toBePreserved, toBeDeleted in
    ...
    public func wantsAccess(to node: Node, accessor: NodeAccessor) {
        // The nodeIsUsable() will be called when the file will be usable.
        if node.authorized.contains("*") || node.authorized.contains(_document.currentUser.UID) {
            if _accessors[node.UID] != nil {
                _accessors[node.UID] = [NodeAccessor]()
            }
            if !_accessors[node.UID]!.contains(where: { $0.UID == accessor.UID }) {
                self._accessors[node.UID]!.append(accessor)
            }
            if isAssembled(node) {
                _grantAccess(to: node, accessor: accessor)
            }
        } else {
    ...
    public func stopsAccessing(to node: Node, accessor: NodeAccessor) {
        if let idx = self._accessors[node.UID]?.index(where: { $0.UID == accessor.UID }) {
            _accessors[node.UID]!.remove(at: idx)
        }
    }
    ...
    fileprivate func _grantAccess(to node: Node, accessor: NodeAccessor) {
        accessor.fileIsAvailable(for: node, at: assemblyPath(for: node))
    }
    ...
    public func copy(node: Node, to relativePath: String, completed: @escaping (Completion) -> Void) {
        // The nodeIsUsable() will be called when the file will be usable.
        if node.authorized.contains("*") || node.authorized.contains(_document.currentUser.UID) {
    ...
    ///   - completed: a closure called on completion with Completion State.
    ///                the copied node UID is stored in the completion.getResultExternalReference()
    public func move(node: Node, to relativePath: String, completed: @escaping (Completion) -> Void) {
        // The nodeIsUsable() will be called when the file will be usable.
        if node.authorized.contains("*") || node.authorized.contains(_document.currentUser.UID) {
    ...
    ///   - node: the node
    ///   - completed: a closure called on completion with Completion State.
    public func delete(node: Node, completed: @escaping (Completion) -> Void) {
        // The nodeIsUsable() will be called when the file will be usable.
        if node.authorized.contains("*") || node.authorized.contains(_document.currentUser.UID) {
    ...
    ///   - relativePath: the relative Path
    ///   - completed: a closure called on completion with Completion State.
    public func createFolder(in _: Box, at _: String, completed: @escaping (Completion) -> Void) {
        /// TODO implement
    ...
    ///   - completed: a closure called on completion with Completion State.
    public func createAlias(of node: Node, to _: String, completed: @escaping (Completion) -> Void) {
        // The nodeIsUsable() will be called when the file will be usable.
        if node.authorized.contains("*") || node.authorized.contains(_document.currentUser.UID) {
            // TODO:
            /// Create the Alias
    ...
    public func triggerWillBeIntegrated(trigger _: Trigger) {
    ...
    ///
    /// - Parameter trigger: the trigger
    public func triggerHasBeenIntegrated(trigger _: Trigger) {
    ...
    internal func triggered_download(block: Block) {
        if let node = block.node {
            if node.authorized.contains(_document.currentUser.UID) ||
                node.authorized.contains("*") {
                _toBeDownloadedBlocksUIDS.append(block.UID)
                _downloadNext()
            }
        }
    }
    ...
    internal func _uploadNext() {
        if _uploadsInProgress.count < _maxSimultaneousOperations {
            do {
                let uploadableBlocks = try _toBeUploadedBlocksUIDS.map({ (UID) -> Block in
                    try Bartleby.registredObjectByUID(UID) as Block
                })
                ...
                for candidate in priorizedBlocks {
                    if _toBeUploadedBlocksUIDS.contains(candidate.UID) && candidate.uploadInProgress == false {
                ...
                func __removeBlockFromList(_ block: Block) {
                    ...
                    if let idx = self._toBeUploadedBlocksUIDS.index(where: { $0 == block.UID }) {
                        _toBeUploadedBlocksUIDS.remove(at: idx)
                    }

                    if let idx = self._uploadsInProgress.index(where: { $0.blockUID == block.UID }) {
                        _uploadsInProgress.remove(at: idx)
                    }
                }
                ...
                if toBeUploaded != nil {
                    if let block = try? Bartleby.registredObjectByUID(toBeUploaded!.UID) as Block {
                        ...
                        let uploadOperation = UploadBlock(block: block, documentUID: _document.UID, sucessHandler: { _ in
                        ...
                        _uploadsInProgress.append(uploadOperation)

                    } else {
                        _document.log("Block not found \(toBeUploaded!.UID)", file: #file, function: #function, line: #line, category: Default.LOG_DEFAULT, decorative: false)
                    }
                }
    ...
    internal func _downloadNext() {
        if _downloadsInProgress.count < _maxSimultaneousOperations {
            do {
                let downloadableBlocks = try _toBeDownloadedBlocksUIDS.map({ (UID) -> Block in
                    try Bartleby.registredObjectByUID(UID) as Block
                })
                ...
                for candidate in priorizedBlocks {
                    if _toBeDownloadedBlocksUIDS.contains(candidate.UID) && candidate.downloadInProgress == false {
                ...
                func __removeBlockFromList(_ block: Block) {
                    ...
                    if let idx = self._toBeDownloadedBlocksUIDS.index(where: { $0 == block.UID }) {
                        _toBeDownloadedBlocksUIDS.remove(at: idx)
                    }

                    if let idx = self._downloadsInProgress.index(where: { $0.blockUID == block.UID }) {
                        _downloadsInProgress.remove(at: idx)
                    }
                }
                ...
                if toBeDownloaded != nil {
                    if let block = try? Bartleby.registredObjectByUID(toBeDownloaded!.UID) as Block {
                        ...
                        let downLoadOperation = DownloadBlock(block: block, documentUID: _document.UID, sucessHandler: { tempURL in
                            var data: Data?
                            var sha1 = ""
                            Async.utility {
                                do {
                        ...
                        _downloadsInProgress.append(downLoadOperation)
                    } else {
                        _document.log("Block not found \(toBeDownloaded!.UID)", file: #file, function: #function, line: #line, category: Default.LOG_DEFAULT, decorative: false)
                    }
                }

            } catch {
                _document.log("\(error)", file: #file, function: #function, line: #line, category: Default.LOG_WARNING, decorative: false)
            }
        }
    }
    ...
    open func deleteBlockFile(_ block: Block) {
        if let node: Node = block.firstRelation(Relationship.ownedBy) {
            ...
            _document.log("Block's node not found (block.UID:\(block.UID)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
        }
        _document.blocks.removeObject(block)
        do {
            try _document.removeBlock(with: block.digest)
        } catch {
            _document.log("\(error)", file: #file, function: #function, line: #line, category: Default.LOG_WARNING, decorative: false)
        }
    }
