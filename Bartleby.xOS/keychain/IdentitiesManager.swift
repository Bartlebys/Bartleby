...
open class IdentitiesManager {
            ...
            do {
                if document.metadata.appGroup != "" {
                    ...
                    for profile in identities.profiles {
                        ...
                        let jsonProfileString = String(data: jsonProfileData, encoding: Default.STRING_ENCODING)
    ...
    ///
    open static func synchronize(_ document: BartlebyDocument, password: String, completed: @escaping (Completion) -> Void) {
        if document.metadata.appGroup != "" {
    ...
    fileprivate static func _syndicateProfiles(_ document: BartlebyDocument) throws {
        ...
        if identities.profiles.contains(where: { (profile) -> Bool in
            profile.user?.UID == currentUser.UID
        }) {
