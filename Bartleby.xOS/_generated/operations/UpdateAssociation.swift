...
@objc(UpdateAssociation) public class UpdateAssociation : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ association:Association, in document:BartlebyDocument){
        let operationInstance=UpdateAssociation()
        operationInstance.referentDocument = document
        operationInstance._payload=association.toJSONString() ?? Default.VOID_STRING
        let context=Context(code:792788568, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        ...
        if let association = Mapper <Association>().map(JSONString:self._payload){
            do{
                ...
                caller: "UpdateAssociation.execute",
                relatedURL:nil,
    ...
    open class func execute(_ association:Association,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpdateAssociation" ,forMethod:"PUT", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 852853405,
                        caller: "UpdateAssociation.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of association",
                            comment: "update of association failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="UpdateAssociation"
