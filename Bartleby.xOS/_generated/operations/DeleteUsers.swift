...
@objc public class DeleteUsers : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ users:[User], from document:BartlebyDocument){
        ...
        let context=Context(code:913851816, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(users.self)
            ...
            pushOperation.quietChanges{
				...
				let stringIDS=PString.ltrim(users.reduce("", { $0+","+$1.UID }),characters:",")
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let users = try JSON.decoder.decode([User].self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("DeleteUsers can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"DeleteUsers: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"DeleteUsers: \(self.documentUID)")
    }
    ...
    open class func execute(_ users:[User],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["ids"] = users.map{$0.UID}
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteUsers" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 2406565946,
                        caller: "DeleteUsers.execute",
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="DeleteUsers"
