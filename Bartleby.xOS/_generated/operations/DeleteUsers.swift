...
@objc public class DeleteUsers: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ users: [User], from document: BartlebyDocument) {
        ...
        let context = Context(code: 913_851_816, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let users = try JSON.decoder.decode([User].self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("DeleteUsers can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "DeleteUsers: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "DeleteUsers: \(documentUID)")
    }
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 2_406_565_946,
                                              caller: "DeleteUsers.execute",
