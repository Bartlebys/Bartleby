...
@objc public class UpsertLocalizedDatum: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ localizedDatum: LocalizedDatum, in document: BartlebyDocument) {
        ...
        let context = Context(code: 1_004_587_844, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let localizedDatum = try JSON.decoder.decode(LocalizedDatum.self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("UpsertLocalizedDatum can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "UpsertLocalizedDatum: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "UpsertLocalizedDatum: \(documentUID)")
    }
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 650_149_607,
                                              caller: "UpsertLocalizedDatum.execute",
