...
@objc public class UpsertLocalizedDatum : ManagedModel,BartlebyOperation{

    // Universal type support
    override open class func typeName() -> String {
        return "UpsertLocalizedDatum"
    }
    ...
    - parameter localizedDatum: the instance
    - parameter document:     the document
    */
    static func commit(_ localizedDatum:LocalizedDatum, in document:BartlebyDocument){
        ...
        let operationInstance = UpsertLocalizedDatum()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:1004587844, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(localizedDatum.self)
            ...
            pushOperation.quietChanges{
				...
				pushOperation.summary="\(operationInstance.runTimeTypeName())(\(localizedDatum.UID))"
                pushOperation.creatorUID = document.metadata.currentUserUID
                operationInstance.creatorUID = document.metadata.currentUserUID
                
				Bartleby.markCommitted(localizedDatum.UID)

            }
            pushOperation.operationName = UpsertLocalizedDatum.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let localizedDatum = try JSON.decoder.decode(LocalizedDatum.self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    type(of: self).execute(localizedDatum,
                        in:self.documentUID,
                    ...
                    self.referentDocument?.log("UpsertLocalizedDatum can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "UpsertLocalizedDatum.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpsertLocalizedDatum: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpsertLocalizedDatum: \(self.documentUID)")
    }

    

    open class func execute(_ localizedDatum:LocalizedDatum,
            in documentUID:String,
            sucessHandler success: @escaping(_ context:HTTPContext)->(),
            failureHandler failure: @escaping(_ context:HTTPContext)->()){
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                let pathURL = document.baseURL.appendingPathComponent("localizedDatum")
                var parameters = [String: Any]()
                parameters["localizedDatum"] = localizedDatum.dictionaryRepresentation()
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpsertLocalizedDatum" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 650149607,
                        caller: "UpsertLocalizedDatum.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of localizedDatum",
                            comment: "creation of localizedDatum failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="UpsertLocalizedDatum"
										    ...
										    acknowledgment.uids=[localizedDatum.UID]
