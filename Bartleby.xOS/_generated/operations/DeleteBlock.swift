...
@objc public class DeleteBlock : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ block:Block, from document:BartlebyDocument){
        ...
        let operationInstance = DeleteBlock()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:1882299827, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.operationName = DeleteBlock.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let block = try JSON.decoder.decode(Block.self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("DeleteBlock can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "DeleteBlock.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"DeleteBlock: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"DeleteBlock: \(self.documentUID)")
    }
    ...
    open class func execute(_ block:Block,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["blockId"] = block.UID
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteBlock" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 89311246,
                        caller: "DeleteBlock.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteByIds  of block",
                            comment: "deleteByIds of block failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="DeleteBlock"
