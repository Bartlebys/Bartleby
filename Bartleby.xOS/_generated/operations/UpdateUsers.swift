...
@objc public class UpdateUsers : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ users:[User], in document:BartlebyDocument){
        ...
        let context=Context(code:3977846924, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("UpdateUsers can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpdateUsers: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpdateUsers: \(self.documentUID)")
    }
    ...
    open class func execute(_ users:[User],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                for user in users{
                    let serializedInstance = user.dictionaryRepresentation()
                    collection.append(serializedInstance)
                }
                parameters["users"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpdateUsers" ,forMethod:"PUT", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 3014249870,
                        caller: "UpdateUsers.execute",
