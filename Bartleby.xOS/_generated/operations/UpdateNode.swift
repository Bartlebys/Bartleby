...
@objc public class UpdateNode : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ node:Node, in document:BartlebyDocument){
        ...
        let context=Context(code:3873361880, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("UpdateNode can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpdateNode: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpdateNode: \(self.documentUID)")
    }
