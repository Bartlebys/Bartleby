...
@objc public class UpdateLocker: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ locker: Locker, in document: BartlebyDocument) {
        ...
        let operationInstance = UpdateLocker()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context = Context(code: 1_710_194_365, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
            ...
            pushOperation.operationName = UpdateLocker.typeName()
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let locker = try JSON.decoder.decode(Locker.self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("UpdateLocker can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "UpdateLocker: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "UpdateLocker: \(documentUID)")
    }
    ...
    open class func execute(_ locker: Locker,
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            let urlRequest = HTTPManager.requestWithToken(inDocumentWithUID: document.UID, withActionName: "UpdateLocker", forMethod: "PUT", and: pathURL)
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 3_808_377_834,
                                              caller: "UpdateLocker.execute",
                            ...
                            if let dictionary = result.value as? Dictionary<String, AnyObject> {
                                if let index = dictionary["triggerIndex"] as? NSNumber,
                                    ...
                                    if index.intValue >= 0 {
                                        ...
                                        acknowledgment.operationName = "UpdateLocker"
