...
@objc public class CreateUser : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ user:User, in document:BartlebyDocument){
        ...
        let operationInstance = CreateUser()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:3114275262, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(user.self)
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let user = try JSON.decoder.decode(User.self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("CreateUser can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"CreateUser: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"CreateUser: \(self.documentUID)")
    }
    ...
    open class func execute(_ user:User,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["user"] = user.dictionaryRepresentation()
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"CreateUser" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 2066359615,
                        caller: "CreateUser.execute",
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="CreateUser"
