...
@objc(CreateUser) public class CreateUser : BartlebyObject,BartlebyOperation{
    ...
    override open func encode(with coder: NSCoder) {
		...
		if let _user = self._user {
			coder.encode(_user,forKey:"_user")
		}
    ...
    */
    ...
    internal func _commit(){
        if let user = self._user{
            let context=Context(code:3114275262, caller: "\(self.runTimeTypeName()).commit")
            if let document = Bartleby.sharedInstance.getDocumentByUID(self._documentUID) {
                ...
                do{
					...
					pushOperation.summary="\(self.runTimeTypeName())(\(user.UID))"
            ...
            glog("_user should not be nil", file: #file, function: #function, line: #line, category: Default.LOG_CATEGORY, decorative: false)
        }
    }
