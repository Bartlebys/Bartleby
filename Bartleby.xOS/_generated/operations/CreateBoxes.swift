...
@objc public class CreateBoxes : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ boxes:[Box], in document:BartlebyDocument){
        ...
        let operationInstance = CreateBoxes()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:1896855256, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(boxes.self)
            ...
            pushOperation.operationName = CreateBoxes.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let boxes = try JSON.decoder.decode([Box].self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("CreateBoxes can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "CreateBoxes.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"CreateBoxes: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"CreateBoxes: \(self.documentUID)")
    }
    ...
    open class func execute(_ boxes:[Box],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                for box in boxes{
                    let serializedInstance = box.dictionaryRepresentation()
                    collection.append(serializedInstance)
                }
                parameters["boxes"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"CreateBoxes" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 3744681717,
                        caller: "CreateBoxes.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of boxes",
                            comment: "creation of boxes failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="CreateBoxes"
