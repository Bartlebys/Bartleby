...
@objc public class CreateBoxes: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ boxes: [Box], in document: BartlebyDocument) {
        ...
        let context = Context(code: 1_896_855_256, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let boxes = try JSON.decoder.decode([Box].self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("CreateBoxes can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "CreateBoxes: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "CreateBoxes: \(documentUID)")
    }
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 3_744_681_717,
                                              caller: "CreateBoxes.execute",
