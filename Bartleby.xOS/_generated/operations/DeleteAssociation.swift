...
@objc(DeleteAssociation) public class DeleteAssociation : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ association:Association, from document:BartlebyDocument){
        let operationInstance=DeleteAssociation()
        operationInstance.referentDocument = document
        operationInstance._payload=association.toJSONString() ?? Default.VOID_STRING
        let context=Context(code:3773369330, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        ...
        if let association = Mapper <Association>().map(JSONString:self._payload){
            do{
                ...
                caller: "DeleteAssociation.execute",
                relatedURL:nil,
    ...
    open class func execute(_ association:Association,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["associationId"]=association.UID
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteAssociation" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 68772254,
                        caller: "DeleteAssociation.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteByIds  of association",
                            comment: "deleteByIds of association failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="DeleteAssociation"
