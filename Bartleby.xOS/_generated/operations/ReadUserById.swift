...
@objc(ReadUserById) public class ReadUserById : JObject{
    ...
    public static func execute(fromRegistry registryUID:String,
						userId:String,
						sucessHandler success:(user:User)->(),
						failureHandler failure:(context:JHTTPResponse)->()){
	

        if let document = Bartleby.sharedInstance.getDocumentByUID(registryUID) {
            let pathURL=document.baseURL.URLByAppendingPathComponent("user/\(userId)")
            let dictionary:Dictionary<String, AnyObject>=[:]
            let urlRequest=HTTPManager.mutableRequestWithToken(inRegistry:document.UID,withActionName:"ReadUserById" ,forMethod:"GET", and: pathURL)
