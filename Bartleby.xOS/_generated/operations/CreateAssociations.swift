...
@objc(CreateAssociations) public class CreateAssociations : ManagedModel,BartlebyOperation{
    ...
    - parameter associations: the instance
    - parameter document:     the document
    */
    static func commit(_ associations:[Association], in document:BartlebyDocument){
        let operationInstance=CreateAssociations()
        operationInstance.referentDocument = document
        operationInstance._payload=associations.toJSONString() ?? Default.VOID_STRING
        let context=Context(code:2414414838, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.quietChanges{
				...
				let stringIDS=PString.ltrim(associations.reduce("", { $0+","+$1.UID }),characters:",")
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        ...
        if let associations = Mapper <Association>().mapArray(JSONString:self._payload){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    type(of: self).execute(associations,
                        in:self.documentUID,
                ...
                caller: "CreateAssociations.execute",
                relatedURL:nil,
            ...
            glog("associations should not be nil", file: #file, function: #function, line: #line, category: Default.LOG_WARNING, decorative: false)
        }
    }
    ...
    open class func execute(_ associations:[Association],
            in documentUID:String,
            sucessHandler success: @escaping(_ context:HTTPContext)->(),
            failureHandler failure: @escaping(_ context:HTTPContext)->()){
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                let pathURL = document.baseURL.appendingPathComponent("associations")
                var parameters=Dictionary<String, Any>()
                var collection=[Dictionary<String, Any>]()
                for association in associations{
                    let serializedInstance=Mapper<Association>().toJSON(association)
                    collection.append(serializedInstance)
                }
                parameters["associations"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"CreateAssociations" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 52143495,
                        caller: "CreateAssociations.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of associations",
                            comment: "creation of associations failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="CreateAssociations"
										...
										acknowledgment.uids=associations.map({$0.UID})
