...
import Foundation
	...
	import ObjectMapper
#endif
@objc(ReadAssociationsByQueryParameters) public class ReadAssociationsByQueryParameters : ManagedModel {
		// Universal type support
	override open class func typeName() -> String {
		 return "ReadAssociationsByQueryParameters"
	}
	// 
...
@objc(ReadAssociationsByQuery) open class ReadAssociationsByQuery : ManagedModel{

    // Universal type support
    override open class func typeName() -> String {
           return "ReadAssociationsByQuery"
    }


    public static func execute(from documentUID:String,
						parameters:ReadAssociationsByQueryParameters,
						sucessHandler success:@escaping(_ associations:[Association])->(),
						failureHandler failure:@escaping(_ context:HTTPContext)->()){
	
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            let pathURL=document.baseURL.appendingPathComponent("associationsByQuery")
            let dictionary:Dictionary<String, Any>?=Mapper().toJSON(parameters)
            let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"ReadAssociationsByQuery" ,forMethod:"GET", and: pathURL)
            ...
            do {
                ...
                request(r).responseString(completionHandler: { (response) in
                    ...
                    let context = HTTPContext( code: 3527412987,
                        caller: "ReadAssociationsByQuery.execute",
					...
					metrics.operationName="ReadAssociationsByQuery"
                        ...
                        failure(context)
                          ...
                          if 200...299 ~= statusCode {
	                            if let string=result.value{
	                                if let instance = Mapper <Association>().mapArray(JSONString:string){
	                                    success(instance)
	                                }else{
	                                    let failureReaction =  Reaction.dispatchAdaptiveMessage(
	                                        context: context,
