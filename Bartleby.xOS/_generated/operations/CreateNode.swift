...
@objc(CreateNode) public class CreateNode : BartlebyObject,BartlebyOperation{
    ...
    override open func encode(with coder: NSCoder) {
		...
		if let _node = self._node {
			coder.encode(_node,forKey:"_node")
		}
    ...
    */
    static func commit(_ node:Node, inDocumentWithUID documentUID:String){
        let operationInstance=CreateNode()
    ...
    func commit(){
        if let node = self._node{
            let context=Context(code:1759518502, caller: "\(self.runTimeTypeName()).commit")
            if let document = Bartleby.sharedInstance.getDocumentByUID(self._documentUID) {
                ...
                do{
					...
					pushOperation.summary="\(self.runTimeTypeName())(\(node.UID))"
            ...
            glog("_node should not be nil", file: #file, function: #function, line: #line, category: Default.LOG_CATEGORY, decorative: false)
        }
    }
