...
@objc public class CreateNode : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ node:Node, in document:BartlebyDocument){
        ...
        let context=Context(code:1759518502, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.operationName = CreateNode.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let node = try JSON.decoder.decode(Node.self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("CreateNode can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"CreateNode: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"CreateNode: \(self.documentUID)")
    }
