...
@objc public class DeleteTags : ManagedModel,BartlebyOperation{
    ...
    - parameter tags: the instance
    - parameter document:     the document
    */
    static func commit(_ tags:[Tag], from document:BartlebyDocument){
        ...
        let operationInstance = DeleteTags()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:3988493671, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(tags.self)
            ...
            pushOperation.operationName = DeleteTags.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let tags = try JSON.decoder.decode([Tag].self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("DeleteTags can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "DeleteTags.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"DeleteTags: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"DeleteTags: \(self.documentUID)")
    }

    

    open class func execute(_ tags:[Tag],
            from documentUID:String,
            sucessHandler success: @escaping(_ context:HTTPContext)->(),
            failureHandler failure: @escaping(_ context:HTTPContext)->()){
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                let pathURL = document.baseURL.appendingPathComponent("tags")
                var parameters = [String: Any]()
                parameters["ids"] = tags.map{$0.UID}
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteTags" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 2222019787,
                        caller: "DeleteTags.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteByIds  of tags",
                            comment: "deleteByIds of tags failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="DeleteTags"
										    ...
										    acknowledgment.uids=tags.map({$0.UID})
