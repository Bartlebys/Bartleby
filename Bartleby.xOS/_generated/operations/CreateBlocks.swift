...
@objc public class CreateBlocks : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ blocks:[Block], in document:BartlebyDocument){
        ...
        let context=Context(code:4192090395, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("CreateBlocks can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"CreateBlocks: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"CreateBlocks: \(self.documentUID)")
    }
