...
@objc(CreateBlocks) public class CreateBlocks : BartlebyObject,BartlebyOperation{
    ...
    override open func encode(with coder: NSCoder) {
		...
		if let _blocks = self._blocks {
			coder.encode(_blocks,forKey:"_blocks")
		}
    ...
    */
    static func commit(_ blocks:[Block], inDocumentWithUID documentUID:String){
        let operationInstance=CreateBlocks()
    ...
    func commit(){
        if let blocks = self._blocks{
            let context=Context(code:4192090395, caller: "\(self.runTimeTypeName()).commit")
            if let document = Bartleby.sharedInstance.getDocumentByUID(self._documentUID) {
            ...
            glog("_blocks should not be nil", file: #file, function: #function, line: #line, category: Default.LOG_CATEGORY, decorative: false)
        }
    }
