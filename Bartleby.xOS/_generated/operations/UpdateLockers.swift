...
@objc public class UpdateLockers: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ lockers: [Locker], in document: BartlebyDocument) {
        ...
        let context = Context(code: 776_462_226, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let lockers = try JSON.decoder.decode([Locker].self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("UpdateLockers can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "UpdateLockers: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "UpdateLockers: \(documentUID)")
    }
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 1_231_400_466,
                                              caller: "UpdateLockers.execute",
