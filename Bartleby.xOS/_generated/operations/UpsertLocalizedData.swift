...
@objc public class UpsertLocalizedData : ManagedModel,BartlebyOperation{

    // Universal type support
    override open class func typeName() -> String {
        return "UpsertLocalizedData"
    }
    ...
    - parameter localizedData: the instance
    - parameter document:     the document
    */
    static func commit(_ localizedData:[LocalizedDatum], in document:BartlebyDocument){
        ...
        let operationInstance = UpsertLocalizedData()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:3620092322, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(localizedData.self)
            ...
            pushOperation.quietChanges{
				...
				let stringIDS=PString.ltrim(localizedData.reduce("", { $0+","+$1.UID }),characters:",")
				...
				for item in localizedData{
					Bartleby.markCommitted(item.UID)
				}

            }
            pushOperation.operationName = UpsertLocalizedData.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let localizedData = try JSON.decoder.decode([LocalizedDatum].self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    type(of: self).execute(localizedData,
                        in:self.documentUID,
                    ...
                    self.referentDocument?.log("UpsertLocalizedData can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "UpsertLocalizedData.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpsertLocalizedData: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpsertLocalizedData: \(self.documentUID)")
    }

    

    open class func execute(_ localizedData:[LocalizedDatum],
            in documentUID:String,
            sucessHandler success: @escaping(_ context:HTTPContext)->(),
            failureHandler failure: @escaping(_ context:HTTPContext)->()){
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                let pathURL = document.baseURL.appendingPathComponent("localizedData")
                var parameters = [String: Any]()
                var collection = [[String: Any]]()
                for localizedDatum in localizedData{
                    let serializedInstance = localizedDatum.dictionaryRepresentation()
                    collection.append(serializedInstance)
                }
                parameters["localizedData"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpsertLocalizedData" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 1866800984,
                        caller: "UpsertLocalizedData.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of localizedData",
                            comment: "creation of localizedData failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="UpsertLocalizedData"
										    ...
										    acknowledgment.uids=localizedData.map({$0.UID})
