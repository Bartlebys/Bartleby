...
@objc public class UpsertLocalizedData: ManagedModel, BartlebyOperation {
    ...
    static func commit(_ localizedData: [LocalizedDatum], in document: BartlebyDocument) {
        ...
        let context = Context(code: 3_620_092_322, caller: "\(operationInstance.runTimeTypeName()).commit")
        do {
    ...
    open func push(sucessHandler success: @escaping (_ context: HTTPContext) -> Void,
        ...
        do {
            let localizedData = try JSON.decoder.decode([LocalizedDatum].self, from: _payload ?? Data())
            ...
            if pushOperation.canBePushed() {
                ...
                referentDocument?.log("UpsertLocalizedData can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
            }
        } catch {
    ...
    internal func _getOperation() throws -> PushOperation {
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID: "UpsertLocalizedData: \(UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID: "UpsertLocalizedData: \(documentUID)")
    }
        ...
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            ...
            do {
                ...
                request(r).responseJSON(completionHandler: { response in
                    ...
                    let context = HTTPContext(code: 1_866_800_984,
                                              caller: "UpsertLocalizedData.execute",
