...
@objc(DeleteNode) public class DeleteNode : BartlebyObject,BartlebyOperation{
    ...
    */
    init (_ node:Node=Node(), from documentUID:String) {
        self._node=node
    ...
    - parameter documentUID:     the document UID
    */
    static func commit(_ node:Node, from documentUID:String){
        let operationInstance=DeleteNode(node,from:documentUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:1014119922, caller: "DeleteNode.commit")
        if let document = Bartleby.sharedInstance.getDocumentByUID(self._documentUID) {
            ...
            do{
				...
				pushOperation.summary="DeleteNode(\(self._node.UID))"
               ...
               document.dispatchAdaptiveMessage(context,
                    ...
                    body: "Operation collection is missing in  DeleteNode",
    ...
    static open func execute(_ node:Node,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["nodeId"]=node.UID
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteNode" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 80083624,
                        caller: "DeleteNode.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteByIds  of node",
                            comment: "deleteByIds of node failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="DeleteNode"
