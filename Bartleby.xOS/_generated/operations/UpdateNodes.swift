...
@objc public class UpdateNodes : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ nodes:[Node], in document:BartlebyDocument){
        ...
        let operationInstance = UpdateNodes()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:320633844, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.operationName = UpdateNodes.typeName()
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("UpdateNodes can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "UpdateNodes.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpdateNodes: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpdateNodes: \(self.documentUID)")
    }
    ...
    open class func execute(_ nodes:[Node],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpdateNodes" ,forMethod:"PUT", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 3985191387,
                        caller: "UpdateNodes.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of nodes",
                            comment: "update of nodes failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="UpdateNodes"
