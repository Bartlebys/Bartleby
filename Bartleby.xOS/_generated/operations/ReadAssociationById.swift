...
@objc(ReadAssociationById) open class ReadAssociationById : ManagedModel{

    // Universal type support
    override open class func typeName() -> String {
           return "ReadAssociationById"
    }


    public static func execute(from documentUID:String,
						associationId:String,
						sucessHandler success:@escaping(_ association:Association)->(),
						failureHandler failure:@escaping(_ context:HTTPContext)->()){
	
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            let pathURL=document.baseURL.appendingPathComponent("association/\(associationId)")
            let dictionary:Dictionary<String, Any>=Dictionary<String, Any>()
            let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"ReadAssociationById" ,forMethod:"GET", and: pathURL)
            ...
            do {
                ...
                request(r).responseString(completionHandler: { (response) in
                    ...
                    let context = HTTPContext( code: 353567617,
                        caller: "ReadAssociationById.execute",
					...
					metrics.operationName="ReadAssociationById"
                        ...
                        failure(context)
                          ...
                          if 200...299 ~= statusCode {
	                            if let string=result.value{
	                                if let instance = Mapper <Association>().map(JSONString:string){
	                                    success(instance)
	                                }else{
	                                    let failureReaction =  Reaction.dispatchAdaptiveMessage(
	                                        context: context,
