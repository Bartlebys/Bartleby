...
import Foundation
	...
	import ObjectMapper
#endif
@objc(ReadBoxesByQueryParameters) public class ReadBoxesByQueryParameters : BartlebyObject {
		// Universal type support
	override open class func typeName() -> String {
		 return "ReadBoxesByQueryParameters"
	}
	// 
...
@objc(ReadBoxesByQuery) open class ReadBoxesByQuery : BartlebyObject{

    // Universal type support
    override open class func typeName() -> String {
           return "ReadBoxesByQuery"
    }


    public static func execute(from documentUID:String,
						parameters:ReadBoxesByQueryParameters,
						sucessHandler success:@escaping(_ boxes:[Box])->(),
						failureHandler failure:@escaping(_ context:HTTPContext)->()){
	
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            let pathURL=document.baseURL.appendingPathComponent("boxesByQuery")
            let dictionary:Dictionary<String, Any>?=Mapper().toJSON(parameters)
            let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"ReadBoxesByQuery" ,forMethod:"GET", and: pathURL)
            ...
            do {
                ...
                request(r).responseString(completionHandler: { (response) in
                    ...
                    let context = HTTPContext( code: 524618339,
                        caller: "ReadBoxesByQuery.execute",
					...
					metrics.operationName="ReadBoxesByQuery"
                        ...
                        failure(context)
                          ...
                          if 200...299 ~= statusCode {
	                            if let string=result.value{
	                                if let instance = Mapper <Box>().mapArray(JSONString:string){
	                                    success(instance)
	                                }else{
	                                    let failureReaction =  Reaction.dispatchAdaptiveMessage(
	                                        context: context,
