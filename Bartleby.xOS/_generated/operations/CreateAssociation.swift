...
@objc(CreateAssociation) public class CreateAssociation : ManagedModel,BartlebyOperation{
    ...
    - parameter association: the instance
    - parameter document:     the document
    */
    static func commit(_ association:Association, in document:BartlebyDocument){
        let operationInstance=CreateAssociation()
        operationInstance.referentDocument = document
        operationInstance._payload=association.toJSONString() ?? Default.VOID_STRING
        let context=Context(code:447243292, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.quietChanges{
				...
				pushOperation.summary="\(operationInstance.runTimeTypeName())(\(association.UID))"
				...
				Bartleby.markCommitted(association.UID)
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        ...
        if let association = Mapper <Association>().map(JSONString:self._payload){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    type(of: self).execute(association,
                        in:self.documentUID,
                ...
                caller: "CreateAssociation.execute",
                relatedURL:nil,
            ...
            glog("association should not be nil", file: #file, function: #function, line: #line, category: Default.LOG_WARNING, decorative: false)
        }
    }
    ...
    open class func execute(_ association:Association,
            in documentUID:String,
            sucessHandler success: @escaping(_ context:HTTPContext)->(),
            failureHandler failure: @escaping(_ context:HTTPContext)->()){
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                let pathURL = document.baseURL.appendingPathComponent("association")
                var parameters=Dictionary<String, Any>()
                parameters["association"]=Mapper<Association>().toJSON(association)
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"CreateAssociation" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 1129443214,
                        caller: "CreateAssociation.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of association",
                            comment: "creation of association failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="CreateAssociation"
										...
										acknowledgment.uids=[association.UID]
