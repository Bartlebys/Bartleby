...
import Foundation
	...
	import ObjectMapper
#endif
@objc(ReadNodesByQueryParameters) public class ReadNodesByQueryParameters : BartlebyObject {
		// Universal type support
	override open class func typeName() -> String {
		 return "ReadNodesByQueryParameters"
	}
	// 
...
@objc(ReadNodesByQuery) open class ReadNodesByQuery : BartlebyObject{

    // Universal type support
    override open class func typeName() -> String {
           return "ReadNodesByQuery"
    }


    public static func execute(from documentUID:String,
						parameters:ReadNodesByQueryParameters,
						sucessHandler success:@escaping(_ nodes:[Node])->(),
						failureHandler failure:@escaping(_ context:HTTPContext)->()){
	
        if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
            let pathURL=document.baseURL.appendingPathComponent("nodesByQuery")
            let dictionary:Dictionary<String, Any>?=Mapper().toJSON(parameters)
            let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"ReadNodesByQuery" ,forMethod:"GET", and: pathURL)
            ...
            do {
                ...
                request(r).responseString(completionHandler: { (response) in
                    ...
                    let context = HTTPContext( code: 668804740,
                        caller: "ReadNodesByQuery.execute",
					...
					metrics.operationName="ReadNodesByQuery"
                        ...
                        failure(context)
                          ...
                          if 200...299 ~= statusCode {
	                            if let string=result.value{
	                                if let instance = Mapper <Node>().mapArray(JSONString:string){
	                                    success(instance)
	                                }else{
	                                    let failureReaction =  Reaction.dispatchAdaptiveMessage(
	                                        context: context,
