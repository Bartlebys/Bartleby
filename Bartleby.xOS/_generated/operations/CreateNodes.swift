...
@objc(CreateNodes) public class CreateNodes : BartlebyObject,BartlebyOperation{
    ...
    fileprivate var _nodes:[Node] = [Node]()
    ...
    override open var exposedKeys:[String] {
        ...
        exposed.append(contentsOf:["_nodes","_documentUID"])
        return exposed
    }
    ...
    required public init?(coder decoder: NSCoder) {
        ...
        self.silentGroupedChanges {
			self._nodes=decoder.decodeObject(of: [NSArray.classForCoder(),Node.classForCoder()], forKey: "_nodes")! as! [Node]
    ...
    override open func encode(with coder: NSCoder) {
		...
		coder.encode(self._nodes,forKey:"_nodes")
    ...
    - parameter nodes: the [Node] concerned the operation
    - parameter documentUID the document UID

    */
    init (_ nodes:[Node]=[Node](), inDocumentWithUID documentUID:String) {
        self._nodes=nodes
    ...
    - parameter nodes: the instance
    - parameter documentUID:     the document UID
    */
    static func commit(_ nodes:[Node], inDocumentWithUID documentUID:String){
        let operationInstance=CreateNodes(nodes,inDocumentWithUID:documentUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:1234672191, caller: "CreateNodes.commit")
        if let document = Bartleby.sharedInstance.getDocumentByUID(self._documentUID) {
            ...
            do{
				...
				let stringIDS=PString.ltrim(self._nodes.reduce("", { $0+","+$1.UID }),characters:",")
				pushOperation.summary="CreateNodes(\(stringIDS))"
               ...
               document.dispatchAdaptiveMessage(context,
                    ...
                    body: "Operation collection is missing in  CreateNodes",
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        ...
        if  pushOperation.canBePushed(){
            ...
            CreateNodes.execute(self._nodes,
    ...
    static open func execute(_ nodes:[Node],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                for node in nodes{
                    let serializedInstance=Mapper<Node>().toJSON(node)
                    collection.append(serializedInstance)
                }
                parameters["nodes"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"CreateNodes" ,forMethod:"POST", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 3781533954,
                        caller: "CreateNodes.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of nodes",
                            comment: "creation of nodes failure description")
                            ...
                            if let dictionary = result.value as? Dictionary< String,AnyObject > {
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										...
										acknowledgment.operationName="CreateNodes"
										...
										acknowledgment.uids=nodes.map({$0.UID})
