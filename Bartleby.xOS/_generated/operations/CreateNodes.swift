...
@objc public class CreateNodes : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ nodes:[Node], in document:BartlebyDocument){
        ...
        let operationInstance = CreateNodes()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:1234672191, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            operationInstance._payload = try JSON.encoder.encode(nodes.self)
            ...
            pushOperation.quietChanges{
				...
				let stringIDS=PString.ltrim(nodes.reduce("", { $0+","+$1.UID }),characters:",")
            ...
            pushOperation.operationName = CreateNodes.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let nodes = try JSON.decoder.decode([Node].self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("CreateNodes can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"CreateNodes: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"CreateNodes: \(self.documentUID)")
    }

    

    open class func execute(_ nodes:[Node],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                for node in nodes{
                    let serializedInstance = node.dictionaryRepresentation()
