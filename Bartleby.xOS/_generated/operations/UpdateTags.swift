...
@objc public class UpdateTags : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ tags:[Tag], in document:BartlebyDocument){
        ...
        let operationInstance = UpdateTags()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:924280653, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.operationName = UpdateTags.typeName()
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("UpdateTags can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "UpdateTags.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpdateTags: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpdateTags: \(self.documentUID)")
    }
    ...
    open class func execute(_ tags:[Tag],
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                for tag in tags{
                    let serializedInstance = tag.dictionaryRepresentation()
                    collection.append(serializedInstance)
                }
                parameters["tags"]=collection
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpdateTags" ,forMethod:"PUT", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 1594896879,
                        caller: "UpdateTags.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of tags",
                            comment: "update of tags failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="UpdateTags"
