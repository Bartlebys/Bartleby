...
@objc public class DeleteLocalizedDatum : ManagedModel,BartlebyOperation{

    // Universal type support
    override open class func typeName() -> String {
        return "DeleteLocalizedDatum"
    }
    ...
    */
    static func commit(_ localizedDatum:LocalizedDatum, from document:BartlebyDocument){
        ...
        let operationInstance = DeleteLocalizedDatum()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:2241463531, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.operationName = DeleteLocalizedDatum.typeName()
        ...
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("DeleteLocalizedDatum can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "DeleteLocalizedDatum.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"DeleteLocalizedDatum: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"DeleteLocalizedDatum: \(self.documentUID)")
    }
    ...
    open class func execute(_ localizedDatum:LocalizedDatum,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["localizedDatumId"] = localizedDatum.UID
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"DeleteLocalizedDatum" ,forMethod:"DELETE", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 1612167761,
                        caller: "DeleteLocalizedDatum.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteByIds  of localizedDatum",
                            comment: "deleteByIds of localizedDatum failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="DeleteLocalizedDatum"
