...
@objc public class UpdateTag : ManagedModel,BartlebyOperation{
    ...
    */
    static func commit(_ tag:Tag, in document:BartlebyDocument){
        ...
        let operationInstance = UpdateTag()
        operationInstance.UID = Bartleby.createUID()
        operationInstance.referentDocument = document
        let context=Context(code:1499582507, caller: "\(operationInstance.runTimeTypeName()).commit")
        do{
            ...
            pushOperation.quietChanges{
				...
				Bartleby.markCommitted(tag.UID)

            }
            pushOperation.operationName = UpdateTag.typeName()
    ...
    open func push(sucessHandler success:@escaping (_ context:HTTPContext)->(),
        failureHandler failure:@escaping (_ context:HTTPContext)->()){
            do{
                let tag = try JSON.decoder.decode(Tag.self, from:self._payload ?? Data())
                ...
                if  pushOperation.canBePushed(){
                    ...
                    self.referentDocument?.log("UpdateTag can't be pushed \(pushOperation.status)", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
                }
            }catch{
                let context = HTTPContext( code:3 ,
                caller: "UpdateTag.execute",
                relatedURL:nil,
    ...
    internal func _getOperation()throws->PushOperation{
        if let document = Bartleby.sharedInstance.getDocumentByUID(self.documentUID) {
            ...
            throw BartlebyOperationError.operationNotFound(UID:"UpdateTag: \(self.UID)")
        }
        throw BartlebyOperationError.documentNotFound(documentUID:"UpdateTag: \(self.documentUID)")
    }
    ...
    open class func execute(_ tag:Tag,
            ...
            if let document = Bartleby.sharedInstance.getDocumentByUID(documentUID) {
                ...
                parameters["tag"] = tag.dictionaryRepresentation()
                let urlRequest=HTTPManager.requestWithToken(inDocumentWithUID:document.UID,withActionName:"UpdateTag" ,forMethod:"PUT", and: pathURL)
                do {
                    ...
                    let context = HTTPContext( code: 4222583605,
                        caller: "UpdateTag.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of tag",
                            comment: "update of tag failure description")
                                ...
                                if let index=dictionary["triggerIndex"] as? NSNumber,
                                    let triggerRelayDuration=dictionary["triggerRelayDuration"] as? NSNumber{
										if index.intValue >= 0 {
										    ...
										    acknowledgment.operationName="UpdateTag"
