...
@objc(BartlebyDocument) open class BartlebyDocument: BXDocument, BoxDelegate {
    ...
    open static let VOLATILE_DISPLAY_DURATION: Double = 3
    ...
    open func moveIsReady(node _: Node, to _: String, proceed: () -> Void) {
    ...
    ///
    /// - Parameter node: the node that will be moved or copied
    open func copyIsReady(node _: Node, to _: String, proceed: () -> Void) {
        // If necessary we can wait
        proceed()
    }
    ...
    open func deletionIsReady(node _: Node, proceed: () -> Void) {
        // If necessary we can wait
        proceed()
    }
    ...
    open func nodeIsReady(node _: Node, proceed: () -> Void) {
        proceed()
    }
    ...
    open func allowReplaceContent(of _: Node, withContentAt _: String, by _: NodeAccessor) -> Bool {
    ...
    open func addDocumentMessagesListener(_ listener: MessageListener) {
        if !_messageListeners.contains(where: { (l) -> Bool in
            listener.UID == l.UID
        }) {
    ...
    open func removeDocumentMessagesListener(_ listener: MessageListener) {
        if let idx = self._messageListeners.index(where: { (l) -> Bool in
            ...
            _messageListeners.remove(at: idx)
        }
    }
    ...
    open func configureSchema() {
        ...
        localizedDatumDefinition.proxy = localizedData
        ...
        lockerDefinition.proxy = lockers
        ...
        pushOperationDefinition.proxy = pushOperations
        ...
        do {
            try metadata.configureSchema(blockDefinition)
            try metadata.configureSchema(boxDefinition)
            try metadata.configureSchema(localizedDatumDefinition)
            try metadata.configureSchema(lockerDefinition)
            try metadata.configureSchema(nodeDefinition)
            try metadata.configureSchema(pushOperationDefinition)
            try metadata.configureSchema(userDefinition)

        } catch let DocumentError.duplicatedCollectionName(collectionName) {
            ...
            log("\(error)", file: #file, function: #function, line: #line, category: Default.LOG_WARNING)
        }
    ...
    open func newManagedModel<T: Collectible>(commit: Bool = true, isUndoable: Bool = true) -> T {
        ...
        if T.typeName() == "User" {
            return _newUser(commit: commit, isUndoable: isUndoable) as! T
        }
        // Generated UnaManaged and ManagedModel are supported
        // We prefer to crash if some tries to inject another collectible
        var instance = try! dynamics.newInstanceOf(T.typeName()) as! T
        register(instance: &instance, commit: commit, isUndoable: isUndoable)
        return instance
    }
    ...
    open func didCreate(_: Collectible) {
    }
