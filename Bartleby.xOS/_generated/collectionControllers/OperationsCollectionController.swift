...
@objc(OperationsCollectionController) public class OperationsCollectionController : JObject,IterableCollectibleCollection{
    ...
    */
    ...
    public func removeObjectFromItemsAtIndex(index: Int, commit:Bool) {
     ...
     public func removeObject(item: Collectible, commit:Bool)->Bool{
        if let instance=item as? Operation{
            #if os(OSX) && !USE_EMBEDDED_MODULES
                if let arrayController = self.arrayController{
                    if let idx=(arrayController.arrangedObjects as? [Operation])?.indexOf({ return $0.UID==instance.UID }){
    ...
    public func removeObjectWithID(id:String, commit:Bool)->Bool{
        #if os(OSX) && !USE_EMBEDDED_MODULES
            if let arrayController = self.arrayController{
                if let idx=(arrayController.arrangedObjects as? [Operation])?.indexOf({ return $0.UID==id }){
                    self.removeObjectFromItemsAtIndex(idx, commit:commit)
                    return true
                }
            }else{
