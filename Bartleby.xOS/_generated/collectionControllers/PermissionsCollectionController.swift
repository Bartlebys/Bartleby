...
@objc(PermissionsCollectionController) public class PermissionsCollectionController : JObject,IterableCollectibleCollection{
    ...
    */
    public func commitChanges() -> [String] {
        ...
        if self.toBeCommitted{ // When one member has to be committed its collection _shouldBeCommited flag is turned to true
            ...
            if  changedItems.count > 0 {
                ...
                UpdatePermissions.commit(changedItems,inRegistryWithUID:self.registryUID)
            }
            self.committed=true
         }
        return UIDS
    ...
    public dynamic var items:[Permission]=[Permission](){
        didSet {
            if items != oldValue {
            ...
            if let arrayController = self.arrayController{
                ...
                if let tableView = self.tableView{
                    dispatch_async(GlobalQueue.Main.get(), {
                        let sorted=self.arrayController?.arrangedObjects as! [Permission]
