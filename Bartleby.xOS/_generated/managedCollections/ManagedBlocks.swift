...
@objc open class ManagedBlocks: ManagedModel, IterableCollectibleCollection {
    open var collectedType: Collectible.Type { return Block.self }
    ...
    // The "blocks" list (computed by _rebuildFromStorage and on operations)
    ...
    // The underlining "blocks" storage (serialized)
    ...
    // So we use a strongly typed `Block for the storage
    ...
    open func replaceProxyData(_ items: [Collectible]) {
        if let collection = items as? [Block] {
            _items = [Block]()
    ...
    public required init(items: [Block], within document: BartlebyDocument) {
        super.init()
    ...
    open func commitChanges() {
        if _staged.count > 0 {
            ...
            if toBeCreated.count > 0 {
                CreateBlocks.commit(toBeCreated, in: referentDocument!)
            }
            if tobeUpdated.count > 0 {
                UpdateBlocks.commit(tobeUpdated, in: referentDocument!)
            }
        ...
        if _deleted.count > 0 {
            ...
            if toBeDeletedBlocks.count > 0 {
                DeleteBlocks.commit(toBeDeletedBlocks, from: referentDocument!)
    ...
    public required init(from decoder: Decoder) throws {
        ...
        try quietThrowingChanges {
            ...
            self._storage = try values.decode([String: Block].self, forKey: ._storage)
    ...
    ///
    /// - Parameters:
    ///   - item: the Block    ///   - commit: should we commit the `Upsertion`?
    /// - Returns: N/A
    ...
    /// Ads an Block    ///
    /// - Parameters:
    ///   - item: the Block    ///   - commit: should we commit the addition?
    ...
    open func append(_ items: [Collectible], commit: Bool, isUndoable: Bool) {
        if let items = items as? [Block] {
            ...
            if commit == true {
                CreateBlocks.commit(items, in: referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit: Bool = true, isUndoable: Bool) {
        if let item = item as? Block {
            ...
            if commit == true {
                CreateBlock.commit(item, in: referentDocument!)
            }
            shouldBeSaved = true
        }
    }
    ...
    open func addObjectFrom(_ data: Data) {
        do {
            if let block: Block = try self.referentDocument?.serializer.deserialize(data, register: true) {
                if let owners = Bartleby.registredManagedModelByUIDs(block.ownedBy) {
                    for owner in owners {
                        // Re associate the relations.
                        if !owner.owns.contains(block.UID) {
                            owner.owns.append(block.UID)
                        }
                    }
                }
                add(block, commit: true, isUndoable: false)
            }
        } catch {
        ...
        set {
            syncOnMain {
                if let blocks = self.selectedBlocks {
                    ...
                    self.referentDocument?.metadata.saveStateOf(_selectedUIDS, identified: self.selectedBlocksUIDSKeys)
                }
            }
        }
        get {
            return syncOnMainAndReturn { () -> [String] in
                self.referentDocument?.metadata.getStateOf(identified: self.selectedBlocksUIDSKeys) ?? [String]()
            }
        }
    }
    ...
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.blocks.arrayController?.setSelectedObjects(blocks)
    @objc open dynamic var selectedBlocks: [Block]? {
        didSet {
            syncOnMain {
                ...
                NotificationCenter.default.post(name: Notification.Name.Blocks.selectionChanged, object: nil)
            }
        }
    }

    // A facility
    open var firstSelectedBlock: Block? { return selectedBlocks?.first }
}
