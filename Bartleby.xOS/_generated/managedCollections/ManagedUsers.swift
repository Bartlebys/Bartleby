...
@objc open class ManagedUsers: ManagedModel, IterableCollectibleCollection {
    open var collectedType: Collectible.Type { return User.self }
    ...
    // The "users" list (computed by _rebuildFromStorage and on operations)
    ...
    // The underlining "users" storage (serialized)
    ...
    // So we use a strongly typed `User for the storage
    ...
    open func replaceProxyData(_ items: [Collectible]) {
        if let collection = items as? [User] {
            _items = [User]()
    ...
    public required init(items: [User], within document: BartlebyDocument) {
        super.init()
    ...
    open func commitChanges() {
        if _staged.count > 0 {
            ...
            for changed in changedUsers {
                if changed.commitCounter > 0 {
                    UpdateUser.commit(changed, in: referentDocument!)
                } else {
                    CreateUser.commit(changed, in: referentDocument!)
                }
            }
            hasBeenCommitted()
            _staged.removeAll()
        ...
        if _deleted.count > 0 {
            ...
            for user in toBeDeletedItems {
                DeleteUser.commit(user, from: referentDocument!)
    ...
    public required init(from decoder: Decoder) throws {
        ...
        try quietThrowingChanges {
            ...
            self._storage = try values.decode([String: User].self, forKey: ._storage)
    ...
    open func append(_ items: [Collectible], commit: Bool, isUndoable: Bool) {
        if let items = items as? [User] {
            ...
            if commit == true {
                CreateUsers.commit(items, in: referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit: Bool = true, isUndoable: Bool) {
        if let item = item as? User {
            ...
            if commit == true {
                CreateUser.commit(item, in: referentDocument!)
            }
            shouldBeSaved = true
        }
    }
    ...
    open func addObjectFrom(_ data: Data) {
        do {
            if let user: User = try self.referentDocument?.serializer.deserialize(data, register: true) {
                ...
                add(user, commit: true, isUndoable: false)
            }
        } catch {
    ...
    fileprivate var _selectedUIDS: [String] {
        set {
            syncOnMain {
                if let users = self.selectedUsers {
                    let _selectedUIDS: [String] = users.map({ (user) -> String in
                        user.UID
                    })
                    self.referentDocument?.metadata.saveStateOf(_selectedUIDS, identified: self.selectedUsersUIDSKeys)
                }
            }
        }
        get {
            return syncOnMainAndReturn { () -> [String] in
                self.referentDocument?.metadata.getStateOf(identified: self.selectedUsersUIDSKeys) ?? [String]()
            }
        }
    }
    ...
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.users.arrayController?.setSelectedObjects(users)
    @objc open dynamic var selectedUsers: [User]? {
        didSet {
            syncOnMain {
                if let users = selectedUsers {
                    let UIDS: [String] = users.map({ (user) -> String in
                        user.UID
                    })
                    self._selectedUIDS = UIDS
                }
                NotificationCenter.default.post(name: Notification.Name.Users.selectionChanged, object: nil)
            }
        }
    }

    // A facility
    open var firstSelectedUser: User? { return selectedUsers?.first }
}
