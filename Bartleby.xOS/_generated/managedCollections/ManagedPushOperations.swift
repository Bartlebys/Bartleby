...
@objc open class ManagedPushOperations: ManagedModel, IterableCollectibleCollection {
    ...
    public required init(items: [PushOperation], within document: BartlebyDocument) {
        super.init()
    ...
    open func propagate() {
        ...
        selectedPushOperations = selected
    }
    ...
    /// - Returns: N/A
    open func append(_ items: [Collectible], commit _: Bool, isUndoable _: Bool) {
    ...
    ///   - isUndoable: is the addition reversible by the undo manager?
    /// - Returns: N/A
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit _: Bool = true, isUndoable _: Bool) {
     ...
     */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit _: Bool = true) {
    ...
    open func addObjectFrom(_ data: Data) {
        do {
            if let pushOperation: PushOperation = try self.referentDocument?.serializer.deserialize(data, register: true) {
                ...
                add(pushOperation, commit: true, isUndoable: false)
            }
        } catch {
    ...
    fileprivate var _selectedUIDS: [String] {
        ...
        get {
            return syncOnMainAndReturn { () -> [String] in
                self.referentDocument?.metadata.getStateOf(identified: self.selectedPushOperationsUIDSKeys) ?? [String]()
            }
        }
    }
    ...
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.pushOperations.arrayController?.setSelectedObjects(pushOperations)
    @objc open dynamic var selectedPushOperations: [PushOperation]? {
        didSet {
            syncOnMain {
    ...
    // A facility
    open var firstSelectedPushOperation: PushOperation? { return selectedPushOperations?.first }
}
