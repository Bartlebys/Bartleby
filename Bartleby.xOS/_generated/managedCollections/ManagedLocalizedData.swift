...
public extension Notification.Name {
    public struct LocalizedData {
        /// Posted when the selected localizedData changed
        public static let selectionChanged = Notification.Name(rawValue: "org.bartlebys.notification.LocalizedData.selectedLocalizedDataChanged")
    }
}

// MARK: A  collection controller of "localizedData"

// This controller implements data automation features.

@objc open class ManagedLocalizedData: ManagedModel, IterableCollectibleCollection {
    open var collectedType: Collectible.Type { return LocalizedDatum.self }

    // Staged "localizedData" identifiers (used to determine what should be committed on the next loop)
    fileprivate var _staged = [String]()

    // Store the  "localizedData" identifiers to be deleted on the next loop
    fileprivate var _deleted = [String]()

    // Ordered UIDS
    fileprivate var _UIDS = [String]()

    // The "localizedData" list (computed by _rebuildFromStorage and on operations)
    @objc fileprivate dynamic var _items = [LocalizedDatum]() {
        didSet {
    ...
    // The underlining "localizedData" storage (serialized)
    ...
    // So we use a strongly typed `LocalizedDatum for the storage
    ...
    public required init(items: [LocalizedDatum], within document: BartlebyDocument) {
        super.init()
    ...
    open func propagate() {
        ...
        selectedLocalizedData = selected
    }
    ...
    open subscript(index: Int) -> LocalizedDatum {
        let key = _UIDS[index]
        return _storage[key]!
    }
    ...
    open func indexOf(element: @escaping (LocalizedDatum) throws -> Bool) rethrows -> Int? {
    ...
    open func commitChanges() {
        if _staged.count > 0 {
            var changedLocalizedData = [LocalizedDatum]()
            for itemUID in _staged {
                if let o: LocalizedDatum = try? Bartleby.registredObjectByUID(itemUID) {
                    changedLocalizedData.append(o)
                }
            }
            if changedLocalizedData.count > 0 {
                UpsertLocalizedData.commit(changedLocalizedData, in: referentDocument!)
            }
        ...
        if _deleted.count > 0 {
            var toBeDeletedLocalizedData = [LocalizedDatum]()
            for itemUID in _deleted {
                if let o: LocalizedDatum = try? Bartleby.registredObjectByUID(itemUID) {
                    toBeDeletedLocalizedData.append(o)
                }
            }
            if toBeDeletedLocalizedData.count > 0 {
                DeleteLocalizedData.commit(toBeDeletedLocalizedData, from: referentDocument!)
                Bartleby.unRegister(toBeDeletedLocalizedData)
            }
            _deleted.removeAll()
        }
    }
    ...
    open override func setExposedValue(_ value: Any?, forKey key: String) throws {
        switch key {
        case "_storage":
            if let casted = value as? [String: LocalizedDatum] {
                _storage = casted
            }
        case "_staged":
    ...
    public required init(from decoder: Decoder) throws {
        ...
        try quietThrowingChanges {
            ...
            self._storage = try values.decode([String: LocalizedDatum].self, forKey: ._storage)
    ...
    ///
    /// - Parameters:
    ///   - item: the LocalizedDatum    ///   - commit: should we commit the `Upsertion`?
    /// - Returns: N/A
    ...
    /// Ads an LocalizedDatum    ///
    /// - Parameters:
    ///   - item: the LocalizedDatum    ///   - commit: should we commit the addition?
    ...
    open func append(_ items: [Collectible], commit: Bool, isUndoable: Bool) {
        if let items = items as? [LocalizedDatum] {
            ...
            if isUndoable {
                ...
                if let undoManager: UndoManager = self.undoManager {
                    ...
                    if !undoManager.isUndoing {
                        undoManager.setActionName(NSLocalizedString("Add LocalizedDatum", comment: "AddLocalizedDatum undo action"))
                    }
                }
            }
            if commit == true {
                UpsertLocalizedData.commit(items, in: referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit: Bool = true, isUndoable: Bool) {
        if let item = item as? LocalizedDatum {
            item.collection = self
            ...
            if commit == true {
                UpsertLocalizedDatum.commit(item, in: referentDocument!)
            }
            shouldBeSaved = true
        }
    }
    ...
    open func removeObjectFromItemsAtIndex(_ index: Int, commit: Bool = true) {
        ...
        let item: LocalizedDatum = self[index]
        ...
        if let undoManager: UndoManager = self.undoManager {
            ...
            if !undoManager.isUndoing {
                undoManager.setActionName(NSLocalizedString("Remove LocalizedDatum", comment: "Remove LocalizedDatum undo action"))
            }
        }
    ...
    open func addObjectFrom(_ data: Data) {
        do {
            if let localizedDatum: LocalizedDatum = try self.referentDocument?.serializer.deserialize(data, register: true) {
                if let owners = Bartleby.registredManagedModelByUIDs(localizedDatum.ownedBy) {
                    for owner in owners {
                        // Re associate the relations.
                        if !owner.owns.contains(localizedDatum.UID) {
                            owner.owns.append(localizedDatum.UID)
                        }
                    }
                }
                add(localizedDatum, commit: true, isUndoable: false)
            }
        } catch {
    ...
    open func removeObject(_ item: Collectible, commit: Bool = true) {
        if let instance = item as? LocalizedDatum {
    ...
    open func filteredCopy(_ isIncluded: (Collectible) -> Bool) -> CollectibleCollection {
        let filteredCollection = ManagedLocalizedData()
        for item in _items {
            if isIncluded(item) {
    ...
    // MARK: - Selection management Facilities
        ...
        open weak var arrayController: NSArrayController? {
            ...
            didSet {
                // self.referentDocument?.setValue(self, forKey: "localizedData")
                arrayController?.objectClass = LocalizedDatum.self
                arrayController?.entityName = LocalizedDatum.className()
        ...
        open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
            ...
            if let keyPath = keyPath, let object = object {
                if keyPath == "selectionIndexes" && (object as? NSArrayController) == arrayController {
                    if let items = self.arrayController?.selectedObjects as? [LocalizedDatum] {
                        selectedLocalizedData = items
                    }
                }
            }
        }
    ...
    fileprivate var _selectedUIDS: [String] {
        ...
        get {
            return syncOnMainAndReturn { () -> [String] in
                self.referentDocument?.metadata.getStateOf(identified: self.selectedLocalizedDataUIDSKeys) ?? [String]()
            }
        }
    }
    ...
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.localizedData.arrayController?.setSelectedObjects(localizedData)
    @objc open dynamic var selectedLocalizedData: [LocalizedDatum]? {
        didSet {
            syncOnMain {
                ...
                NotificationCenter.default.post(name: Notification.Name.LocalizedData.selectionChanged, object: nil)
            }
        }
    }

    // A facility
    open var firstSelectedLocalizedDatum: LocalizedDatum? { return selectedLocalizedData?.first }
}
