...
@objc open class ManagedLocalizedData : ManagedModel,IterableCollectibleCollection{
    ...
    required public init(items:[LocalizedDatum], within document:BartlebyDocument) {
        super.init()
    ...
    */
    ...
    open func commitChanges(){
        if self._staged.count>0{
			...
			if changedLocalizedData.count > 0 {
			    UpsertLocalizedData.commit(changedLocalizedData,in:self.referentDocument!)
			}
        ...
        if self._deleted.count > 0 {
            ...
            if toBeDeletedLocalizedData.count > 0 {
                DeleteLocalizedData.commit(toBeDeletedLocalizedData, from: self.referentDocument!)
    ...
    open func append(_ items:[Collectible],commit:Bool, isUndoable:Bool){
        if let items  = items as? [LocalizedDatum] {
            ...
            if commit==true {
               UpsertLocalizedData.commit(items, in:self.referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit:Bool=true,isUndoable:Bool) {
        if let item = item as? LocalizedDatum{
            ...
            if commit==true {
               UpsertLocalizedDatum.commit(item, in:self.referentDocument!)
            }
            self.shouldBeSaved = true
        }
    }
    ...
    open func addObjectFrom(_ data:Data){
        do{
            if let localizedDatum:LocalizedDatum = try self.referentDocument?.serializer.deserialize(data,register:true){
                ...
                self.add(localizedDatum, commit: true, isUndoable:false)
            }
        }catch{
        ...
        if let keyPath = keyPath, let object = object {
            if keyPath=="selectionIndexes" &&  (object as? NSArrayController) == self.arrayController {
                if let items = self.arrayController?.selectedObjects as? [LocalizedDatum] {
                    self.selectedLocalizedData=items
                }
            }
        }
    }
