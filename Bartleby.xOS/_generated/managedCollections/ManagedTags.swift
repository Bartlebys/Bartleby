...
public extension Notification.Name {
    public struct Tags {
        ...
        public static let selectionChanged = Notification.Name(rawValue: "org.bartlebys.notification.Tags.selectedTagsChanged")
    }
}


// MARK: A  collection controller of "tags"
...
@objc open class ManagedTags : ManagedModel,IterableCollectibleCollection{
    ...
    */
    ...
    open func commitChanges(){
        if self._staged.count>0{
			...
			if tobeUpdated.count > 0 {
			    UpdateTags.commit(tobeUpdated, in:self.referentDocument!)
			}
    ...
    required public init(from decoder: Decoder) throws{
        ...
        try self.quietThrowingChanges {
			...
			self._storage = try values.decode([String:Tag].self,forKey:._storage)
    ...
    ///
    /// - Parameters:
    ///   - item: the Tag    ///   - commit: should we commit the `Upsertion`?
    /// - Returns: N/A
    ...
    /// Ads an Tag    ///
    /// - Parameters:
    ///   - item: the Tag    ///   - commit: should we commit the addition?
            ...
            if isUndoable{
                ...
                if let undoManager: UndoManager = self.undoManager {
                    ...
                    if !undoManager.isUndoing {
                        undoManager.setActionName(NSLocalizedString("Add Tag", comment: "AddTag undo action"))
                    }
                }
            }
            
            if commit==true {
               CreateTags.commit(items, in:self.referentDocument!)
            }
            self.shouldBeSaved = true
        }
    }
    ...
    */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit:Bool=true) {
      ...
      // Add the inverse of this invocation to the undo stack
        if let undoManager: UndoManager = self.undoManager {
            ...
            if !undoManager.isUndoing {
                undoManager.setActionName(NSLocalizedString("Remove Tag", comment: "Remove Tag undo action"))
            }
        }
    ...
    open func addObjectFrom(_ data:Data){
        do{
            if let tag:Tag = try self.referentDocument?.serializer.deserialize(data,register:true){
                if let owners = Bartleby.registredManagedModelByUIDs(tag.ownedBy){
                    for owner in owners{
                        // Re associate the relations.
                        if !owner.owns.contains(tag.UID){
                            owner.owns.append(tag.UID)
                        }
                    }
                }
                self.add(tag, commit: true, isUndoable:false)
            }
        }catch{
    ...
    open weak var arrayController:NSArrayController? {
        ...
        didSet{
            ...
            if let data = self.referentDocument?.metadata.statesDictionary[self.selectedTagsIndexesKey]{
    ...
    // Do not use document.tags.selectedTags=tags
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let keyPath = keyPath, let object = object {
            if keyPath=="selectionIndexes" &&  (object as? NSArrayController) == self.arrayController {
                if let items = self.arrayController?.selectedObjects as? [Tag] {
                    self.selectedTags=items
                }
            }
        }
    }
        ...
        didSet{
            Bartleby.syncOnMain {
                if let tags = selectedTags {
                    ...
                    self.referentDocument?.metadata.statesDictionary[selectedTagsIndexesKey] = encodedIndexes
                }
    ...
    // A facility
    open var firstSelectedTag:Tag? { return self.selectedTags?.first }
