...
@objc(BoxesManagedCollection) open class BoxesManagedCollection : Node,IterableCollectibleCollection{
    ...
    required public init(items:[Box]) {
        super.init()
        self._items=items
    }
    ...
    fileprivate dynamic var _items:[Box]=[Box](){
        didSet {
            if _items != oldValue {
    ...
    */
        ...
        if self.shouldBeCommitted{
            ...
            for changed in changedItems{
				...
				if toBeCreated.count > 0 {
				    CreateBoxes.commit(toBeCreated, inDocumentWithUID:self.documentUID)
				}
				if tobeUpdated.count > 0 {
				    UpdateBoxes.commit(tobeUpdated, inDocumentWithUID:self.documentUID)
				}
    ...
    required public init?(coder decoder: NSCoder) {
        ...
        self.silentGroupedChanges {
			self._items=decoder.decodeObject(of: [NSArray.classForCoder(),Box.classForCoder()], forKey: "_items")! as! [Box]
        }
    }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit:Bool=true) {
        if let item=item as? Box{
            ...
            if let arrayController = self.arrayController{
                ...
                if let tableView = self.tableView{
                    DispatchQueue.main.async(execute: {
                        let sorted=self.arrayController?.arrangedObjects as! [Box]
            ...
            if item.committed==false && commit==true {
               CreateBox.commit(item, inDocumentWithUID:self.documentUID)
            }
    ...
    */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit:Bool=true) {
       let item : Box =  self[index]
        ...
        if commit==true{
            DeleteBox.commit(item,from:self.documentUID) 
        }
    }
