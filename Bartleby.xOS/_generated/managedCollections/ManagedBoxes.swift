...
@objc(ManagedBoxes) open class ManagedBoxes : ManagedModel,IterableCollectibleCollection{
    ...
    fileprivate dynamic var _items=[Box]()  {
        didSet {
    ...
    */
    ...
    open func commitChanges(){
        if self._staged.count>0{
            let changedItems=self._staged.map({ (UID) -> Box in
                let box:Box = try! Bartleby.registredObjectByUID(UID)
                return box
            })
        ...
        if self._deleted.count > 0 {
            let boxes:[Box] = try! Bartleby.registredObjectsByUIDs(self._deleted)
            DeleteBoxes.commit(boxes, from: self.referentDocument!)
            Bartleby.unRegister(boxes)
        ...
        if let item = item as? Box{
            ...
            if let undoManager: UndoManager = undoManager {
                ...
                if !undoManager.isUndoing {
                    undoManager.setActionName(NSLocalizedString("AddBox", comment: "AddBox undo action"))
                }
            }
    ...
    */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit:Bool=true) {
        ...
        if let undoManager: UndoManager = undoManager {
            ...
            if !undoManager.isUndoing {
                undoManager.setActionName(NSLocalizedString("RemoveBox", comment: "Remove Box undo action"))
            }
        }
