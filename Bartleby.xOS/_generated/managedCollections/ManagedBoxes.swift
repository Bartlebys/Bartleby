...
@objc open class ManagedBoxes: ManagedModel, IterableCollectibleCollection {
    ...
    public required init(items: [Box], within document: BartlebyDocument) {
        super.init()
    ...
    open func commitChanges() {
        if _staged.count > 0 {
            ...
            if toBeCreated.count > 0 {
                CreateBoxes.commit(toBeCreated, in: referentDocument!)
            }
            if tobeUpdated.count > 0 {
                UpdateBoxes.commit(tobeUpdated, in: referentDocument!)
            }
        ...
        if _deleted.count > 0 {
            ...
            if toBeDeletedBoxes.count > 0 {
                DeleteBoxes.commit(toBeDeletedBoxes, from: referentDocument!)
    ...
    open func append(_ items: [Collectible], commit: Bool, isUndoable: Bool) {
        if let items = items as? [Box] {
            ...
            if commit == true {
                CreateBoxes.commit(items, in: referentDocument!)
            }
    ...
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit: Bool = true, isUndoable: Bool) {
        if let item = item as? Box {
            ...
            if commit == true {
                CreateBox.commit(item, in: referentDocument!)
            }
            shouldBeSaved = true
        }
    }
    ...
    open func addObjectFrom(_ data: Data) {
        do {
            if let box: Box = try self.referentDocument?.serializer.deserialize(data, register: true) {
                ...
                add(box, commit: true, isUndoable: false)
            }
        } catch {
    ...
    fileprivate var _selectedUIDS: [String] {
        ...
        get {
            return syncOnMainAndReturn { () -> [String] in
                self.referentDocument?.metadata.getStateOf(identified: self.selectedBoxesUIDSKeys) ?? [String]()
            }
        }
    }
    ...
    // to modify the current selection you should use the array controller
    // e.g: referentDocument.boxes.arrayController?.setSelectedObjects(boxes)
    @objc open dynamic var selectedBoxes: [Box]? {
        didSet {
            syncOnMain {
    ...
    // A facility
    open var firstSelectedBox: Box? { return selectedBoxes?.first }
}
