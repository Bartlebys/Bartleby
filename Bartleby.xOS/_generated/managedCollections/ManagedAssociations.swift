...
public extension Notification.Name {
    public struct Associations {
        /// Posted when the selected associations changed
        public static let selectionChanged = Notification.Name(rawValue: "org.bartlebys.notification.Associations.selectedAssociationsChanged")
    }
}


// MARK: A  collection controller of "associations"

// This controller implements data automation features.

@objc(ManagedAssociations) open class ManagedAssociations : ManagedModel,IterableCollectibleCollection{

    // Staged "associations" identifiers (used to determine what should be committed on the next loop)
    fileprivate dynamic var _staged=[String]()

    // Store the  "associations" identifiers to be deleted on the next loop
    fileprivate var _deleted=[String]()

    // Ordered UIDS
    fileprivate var _UIDS=[String]()

    // The underlining "associations" list
    fileprivate dynamic var _items=[Association]()  {
        didSet {
    ...
    // The underlining "associations" storage
    fileprivate var _storage=[String:Association]()

    fileprivate func _rebuildFromStorage(){
        self._UIDS=[String]()
        self._items=[Association]()
    ...
    required public init(items:[Association], within document:BartlebyDocument) {
        super.init()
    ...
    open func propagateCollection(){
        ...
        if self.referentDocument == nil{
            glog("Document Reference is nil during Propagation on ManagedAssociations", file: #file, function: #function, line: #line, category: Default.LOG_FAULT, decorative: false)
        }
        #endif
        self.forEach {
            $0.collection=self
    ...
    open func generate() -> AnyIterator<Association> {
        var nextIndex = -1
    ...
    open subscript(index: Int) -> Association {
    ...
    open func indexOf(element:@escaping(Association) throws -> Bool) rethrows -> Int?{
    ...
    */
    ...
    open func commitChanges(){
        if self._staged.count>0{
            let changedItems=self._staged.map({ (UID) -> Association in
                let association:Association = try! Bartleby.registredObjectByUID(UID)
                return association
            })
			...
			if toBeCreated.count > 0 {
			    CreateAssociations.commit(toBeCreated, in:self.referentDocument!)
			}
			if tobeUpdated.count > 0 {
			    UpdateAssociations.commit(tobeUpdated, in:self.referentDocument!)
			}
        ...
        if self._deleted.count > 0 {
            let associations:[Association] = try! Bartleby.registredObjectsByUIDs(self._deleted)
            DeleteAssociations.commit(associations, from: self.referentDocument!)
            Bartleby.unRegister(associations)
    ...
    override open func setExposedValue(_ value:Any?, forKey key: String) throws {
        switch key {
            case "_storage":
                if let casted=value as? [String:Association]{
    ...
    required public init?(coder decoder: NSCoder) {
        ...
        self.quietChanges {
            self._storage=decoder.decodeObject(of: [NSDictionary.classForCoder(),NSString.self,Association.classForCoder()], forKey: "_storage")! as! [String:Association]
    ...
    */
    open func insertObject(_ item: Collectible, inItemsAtIndex index: Int, commit:Bool=true) {
        if let item = item as? Association{
            ...
            if let undoManager: UndoManager = undoManager {
                ...
                if !undoManager.isUndoing {
                    undoManager.setActionName(NSLocalizedString("AddAssociation", comment: "AddAssociation undo action"))
                }
            }
            ...
            if commit==true {
               CreateAssociation.commit(item, in:self.referentDocument!)
            }
    ...
    */
    open func removeObjectFromItemsAtIndex(_ index: Int, commit:Bool=true) {
        let item : Association =  self[index]
        ...
        if let undoManager: UndoManager = undoManager {
            ...
            if !undoManager.isUndoing {
                undoManager.setActionName(NSLocalizedString("RemoveAssociation", comment: "Remove Association undo action"))
            }
        }
    ...
    open func removeObject(_ item: Collectible, commit:Bool=true){
        if let instance=item as? Association{
    ...
    open weak var arrayController:NSArrayController? {
        ...
        didSet{
            //self.referentDocument?.setValue(self, forKey: "associations")
            arrayController?.objectClass=Association.self
            arrayController?.entityName=Association.className()
            ...
            if let indexes=self.referentDocument?.metadata.stateDictionary[self.selectedAssociationsIndexesKey] as? [Int]{
    ...
    // e.g: referentDocument.associations.arrayController?.setSelectedObjects(associations)
    // Do not use document.associations.selectedAssociations=associations
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let keyPath = keyPath, let object = object {
            if keyPath=="selectionIndexes" &&  (object as? NSArrayController) == self.arrayController {
                if let items = self.arrayController?.selectedObjects as? [Association] {
                    self.selectedAssociations=items
                }
            }
        }
    }
    ...
    open let selectedAssociationsIndexesKey="selectedAssociationsIndexesKey"

    dynamic open var selectedAssociations:[Association]?{
        didSet{
            if let associations = selectedAssociations {
                 let indexes:[Int]=associations.map({ (association) -> Int in
                    return associations.index(where:{ return $0.UID == association.UID })!
                })
                self.referentDocument?.metadata.stateDictionary[selectedAssociationsIndexesKey]=indexes
                NotificationCenter.default.post(name:NSNotification.Name.Associations.selectionChanged, object: nil)
            }
        }
    }

    // A facility
    open var firstSelectedAssociation:Association? { return self.selectedAssociations?.first }
