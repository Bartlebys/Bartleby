...
public struct I18N {
    ...
    public static var languages: [Language] {
        ...
        let sortedLangages = languages.sorted(by: { (lLang, rLang) -> Bool in
            lLang.localizedDisplayName.compare(rLang.localizedDisplayName) == .orderedAscending
        })
...
open class Localized {
    ...
    open func getLanguageCodesForkey(key: String) -> [String] {
        let localizedDatas: [LocalizedDatum] = _reference.relations(Relationship.owns)
        ...
        if _reference.exposedKeys.contains(key) {
            languageCodes.append(_reference.languageCode)
        }
    ...
    open func setString(key: String, stringValue: String, languageCode: String) {
        ...
        do {
            if languageCode == _reference.languageCode {
                try _reference.setExposedValue(stringValue, forKey: key)
                return
            }
        } catch {}
        ...
        if let localizedDatum: LocalizedDatum = localizedDatas.first(where: { (datum) -> Bool in
            datum.key == key && datum.languageCode == languageCode
        }) {
        ...
        if let localizedDatum: LocalizedDatum = self._reference.referentDocument?.newManagedModel() {
            ...
            _reference.declaresOwnership(of: localizedDatum)
        }
    }
    ...
    open func getString(key: String, languageCode: String) -> String? {
        if _reference.languageCode == languageCode {
            do { return try _reference.getExposedValueForKey(key) as? String } catch {}
        }
        let localizedDatas: [LocalizedDatum] = _reference.relations(Relationship.owns)
        if let localizedDatum: LocalizedDatum = localizedDatas.first(where: { (datum) -> Bool in
            datum.key == key && datum.languageCode == languageCode
    ...
    open func getStringOrBaseValue(key: String, languageCode: String) -> String? {
        ...
        return (try? _reference.getExposedValueForKey(key)) as? String
    }
    ...
    /// - Returns: the strings
    open func getStringList(key _: String) -> [String] {
        let localizedDatas: [LocalizedDatum] = _reference.relations(Relationship.owns)
        let filtered = localizedDatas.filter { (datum) -> Bool in
    ...
    open func setData(key: String, dataValue: Data, languageCode: String) {
        ...
        do {
            if languageCode == _reference.languageCode {
                try _reference.setExposedValue(dataValue, forKey: key)
                return
            }
        } catch {}
    ...
    open func getData(key: String, languageCode: String) -> Data? {
        if _reference.languageCode == languageCode {
            do { return try _reference.getExposedValueForKey(key) as? Data } catch {}
        }
        let localizedDatas: [LocalizedDatum] = _reference.relations(Relationship.owns)
        if let localizedDatum: LocalizedDatum = localizedDatas.first(where: { (datum) -> Bool in
            datum.key == key && datum.languageCode == languageCode
    ...
    open func getDataOrBaseValue(key: String, languageCode: String) -> Data? {
        ...
        return (try? _reference.getExposedValueForKey(key)) as? Data
    }
    ...
    /// - Parameter key:
    /// - Returns:
    open func getDataList(key _: String) -> [Data] {
        let localizedDatas: [LocalizedDatum] = _reference.relations(Relationship.owns)
        let filtered = localizedDatas.filter { (datum) -> Bool in
