...
extension BartlebyDocument: Logger {
    ...
    open func log(_ message: Any, file: String = #file, function: String = #function, line: Int = #line, category: String = Default.LOG_DEFAULT, decorative: Bool = false) {
        if enableLog {
            ...
            let entry = LogEntry(counter: logs.count + 1, message: "\(message)", file: file, function: function, line: line, category: category, elapsed: elapsed, decorative: decorative)
            logs.insert(entry, at: 0)
            for observers in logsObservers {
                observers.receive(entry)
            }
            if printLogsToTheConsole {
                Swift.print(entry)
            }
        }
    }
    ...
    open func getLogs(_ matching: @escaping (_ entry: LogEntry) -> Bool) -> String {
        let entries = logs.filter { (entry) -> Bool in
