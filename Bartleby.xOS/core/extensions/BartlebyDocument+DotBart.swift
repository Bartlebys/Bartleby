...
extension BartlebyDocument {
        ...
        public func saveMetadata(handlers: Handlers) {
            ...
            savePanel.nameFieldStringValue = fileURL?.lastPathComponent ?? "metadata"
    ...
    public func exportMetadataTo(_ path: String, handlers: Handlers) {
        do {
            let data = try _getSerializedMetadata()
    ...
    fileprivate func _getSerializedMetadata() throws -> Data {
        let serializedMetadata = try metadata.toCryptedData()
        return serializedMetadata
    }

    fileprivate func _useSerializedMetadata(_ data: Data) throws {
        metadata = try DocumentMetadata.fromCryptedData(data, document: self)
    }
}
