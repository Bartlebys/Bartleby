...
extension BartlebyDocument {
    ...
    internal func _triggersHasBeenReceived(_ triggers: [Trigger]) {
        let nb = metadata.receivedTriggers.count
        for trigger in triggers {
            // We store the triggers index for debug history
            metadata.triggersIndexesDebugHistory.append(trigger.index)
            // We don't want to keep useless triggers that could be received during a divergence resolution.
            if trigger.index > metadata.lastIntegratedTriggerIndex &&
                !(metadata.receivedTriggers.contains(where: { trigger.index == $0.index })) {
                self.metadata.receivedTriggers.append(trigger)
            }
        }

        if metadata.receivedTriggers.count > nb {
            // If some new trigger have been received, we sorte the receivedTrigger list
            metadata.receivedTriggers = metadata.receivedTriggers.sorted { (lTrigger, rTrigger) -> Bool in
                lTrigger.index < rTrigger.index
            }
        }
        ...
        for trigger in triggers {
            ...
            triggerMetrics.httpContext = HTTPContext(code: 0, caller: "TriggerHasBeenReceived", relatedURL: sseURL, httpStatusCode: 0, responseString: s)
            ...
            report(triggerMetrics)
        }

        _integrateContiguousData()
    }
    ...
    public func record(_ ack: Acknowledgment) {
        metadata.triggersIndexesDebugHistory.append(ack.triggerIndex)
        metadata.ownedTriggersIndexes.append(ack.triggerIndex)

        if ack.triggerIndex == metadata.lastIntegratedTriggerIndex + 1 {
            // This index is contigous.
            metadata.lastIntegratedTriggerIndex = ack.triggerIndex
            _integrateContiguousData()
        } else {
            ...
            let fromIndex = metadata.lastIntegratedTriggerIndex + 1
            log("Trying to resolve possible divergences from index \(fromIndex)", file: #file, function: #function, line: #line, category: logsCategoryFor(Trigger.self), decorative: false)
            TriggersAfterIndex.execute(from: UID, index: fromIndex, sucessHandler: { triggers in
    ...
    fileprivate func _integrateContiguousData() {
        ...
        for trigger in metadata.receivedTriggers {
            ...
            if trigger.index == (metadata.lastIntegratedTriggerIndex + 1) {
                _integrate(trigger)
                toBeRemovedIndexes.append(idx)
            } else {
                break
        ...
        for idx in toBeRemovedIndexes.reversed() {
            metadata.receivedTriggers.remove(at: idx)
        }

        // QA check if we encounter data Holes.
        // If a trigger is missing try to fill the hole
        let missing = _missingContiguousTriggersIndexes()
        if missing.count > 0 {
            let s = missing.map({ "\($0)" })
            log("Trying to fill a data hole for index(es): \(s)", file: #file, function: #function, line: #line, category: logsCategoryFor(Trigger.self), decorative: false)
            TriggersForIndexes.execute(from: UID, indexes: missing, sucessHandler: { triggers in
                self._triggersHasBeenReceived(triggers)
            }) { context in
                if context.httpStatusCode != 404 {
                    // What to do on failure ?
        ...
        if trigger.action.contains("Delete") {
            ...
            if let dataPayloads = trigger.payloads,
                ...
                do {
                    ...
                    log("Deserialization exception \(error)", file: #file, function: #function, line: #line, category: logsCategoryFor(Trigger.self), decorative: false)
                }
            }
        }
        metadata.lastIntegratedTriggerIndex = trigger.index
    ...
    public func forceDataIntegration() {
        ...
        metadata.lastIntegratedTriggerIndex = _maxTriggerIndex()
    }

    fileprivate func _maxTriggerIndex() -> Int {
        let highestTriggerIndex: Int = metadata.receivedTriggers.last?.index ?? 0
        let higestOwned: Int = metadata.ownedTriggersIndexes.max() ?? 0
    ...
    public func loadNewTriggers() {
        ...
        TriggersAfterIndex.execute(from: UID, index: metadata.lastIntegratedTriggerIndex, sucessHandler: { triggers in
            self._triggersHasBeenReceived(triggers)
        }) { _ in
            // What to do on failure
        }
    ...
    fileprivate func _missingContiguousTriggersIndexes() -> [Int] {
        ...
        let nextIndexToBeIntegrated = metadata.lastIntegratedTriggerIndex + 1
        let maxIndex = _maxTriggerIndex()
        if nextIndexToBeIntegrated <= maxIndex {
            for index in nextIndexToBeIntegrated ... maxIndex {
                if (!metadata.receivedTriggers.contains(where: { $0.index == index })) &&
                    !metadata.ownedTriggersIndexes.contains(index) {
    ...
    open func getTriggerBufferInformations() -> String {
        var informations = "\nLast integrated trigger Index: \(metadata.lastIntegratedTriggerIndex)\n"
        // Missing
        let missing = _missingContiguousTriggersIndexes()
        ...
        // Data buffer
        informations += "\n"
        informations += "Triggers to be integrated (\(metadata.receivedTriggers.count)):\n"
        for trigger in metadata.receivedTriggers {
        ...
        // Owned Indexes
        informations += "\n"
        let ownedTriggersIndexes = metadata.ownedTriggersIndexes
        informations += "Owned Indexes (\(ownedTriggersIndexes.count)): "
        informations += ownedTriggersIndexes.reduce("", { (string, index) -> String in
            "\(string) \(index)"
        })

        if metadata.debugTriggersHistory {
            // Owned Indexes
            informations += "\n\n"
            let history = metadata.triggersIndexesDebugHistory
    ...
    internal func _connectToSSE() {
        log("SSE is transitioning online", file: #file, function: #function, line: #line, category: "SSE")
