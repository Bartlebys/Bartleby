...
extension BartlebyDocument: ConcreteConsignee, ConcreteTracker, Consignation, AdaptiveConsignation {
    ...
    open func perform(_ reaction: Reaction, forContext _: Consignable) {
        switch reaction {
        case Reaction.nothing:
            break
        case let .dispatchAdaptiveMessage(context, title, body, trigger):
            dispatchAdaptiveMessage(context, title: title, body: body, onSelectedIndex: trigger)
        case let .track(result, context):
            track(result, context: context)
        case let .presentInteractiveMessage(title, body, trigger):
            presentInteractiveMessage(title, body: body, onSelectedIndex: trigger)
        case let .presentVolatileMessage(title, body):
            presentVolatileMessage(title, body: body)
        case let .putMessageInLogs(title, body):
            putMessageInLogs(title, body: body)
        }
    }
    ...
    open func perform(_ reactions: [Reaction], forContext: Consignable) {
        for reaction in reactions {
            perform(reaction, forContext: forContext)
        }
    }
    ...
    open func dispatchAdaptiveMessage(_: Consignable, title: String, body: String, onSelectedIndex: @escaping (_ selectedIndex: UInt) -> Void) {
        // You can override t Consignee and implement your own adaptive mapping
        presentInteractiveMessage(title, body: body, onSelectedIndex: onSelectedIndex)
    ...
    open func presentInteractiveMessage(_ title: String, body: String, onSelectedIndex: @escaping (_ selectedIndex: UInt) -> Void) {
        #if os(OSX)
            ...
            if let popPresenter = alert.popoverPresentationController {
                ...
                popPresenter.sourceRect = rootViewController.view.bounds
            }
