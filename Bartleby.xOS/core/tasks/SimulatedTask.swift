...
public class  SimulatedTask:Task, ConcreteTask {
    ...
    override public func invoke() {
        ...
        if let name: ArgumentType = try? self.arguments() {
            ...
            // "Internal" Progressions of the task.
            let steps = arc4random_uniform(20)
            var d:UInt64 = 0 // 1 s / 1000
            var counter = 0
            for _ in 0 ... steps {
                d = d + UInt64(arc4random())
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(Int64)(d * NSEC_PER_SEC)), dispatch_get_main_queue(), {
                    counter += 1
