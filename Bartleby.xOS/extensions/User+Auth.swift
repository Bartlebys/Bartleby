...
extension User {
    ...
    open func signInURL(for document: BartlebyDocument) -> URL? {
        let password = cryptoPassword
        if let encoded = password.data(using: Default.STRING_ENCODING)?.base64EncodedString() {
            let signin = document.baseURL.absoluteString.replacingOccurrences(of: "/api/v1", with: "") + "/signIn?spaceUID=\(document.spaceUID)&userUID=\(UID)&password=\(encoded)&observationUID=\(document.UID)"
    ...
    open func login(sucessHandler success: @escaping () -> Void,
    ...
    open func logout(sucessHandler success: @escaping () -> Void,
                     failureHandler failure: @escaping (_ context: HTTPContext) -> Void) {
    ...
    open var fullPhoneNumber: String {
        ...
        if let match = self.phoneCountryCode.range(of: "(?<=\\()[^()]{1,10}(?=\\))", options: .regularExpression) {
            prefix = String(phoneCountryCode[match])
        }
