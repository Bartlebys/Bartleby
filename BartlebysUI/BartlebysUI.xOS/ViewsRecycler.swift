...
open class ViewsRecycler {
    ...
    open func stats() -> (total: Int, used: Int, available: Int) {
        let total = _viewsReferers.count
        var used = 0
        var available = 0
        for vs in _viewsReferers {
            if vs.available {
                available += 1
            } else {
                used += 1
    ...
    open func liberate(viewsGroupedBy groupNames: [String]) {
        let referers = _viewsReferers.filter({ (referer) -> Bool in
            groupNames.contains(referer.groupName)
        })
    ...
    open func getAllActiveViews(groupedBy groupNames: [String]) -> [XView] {
        return _viewsReferers.compactMap({ (referer) -> XView? in
