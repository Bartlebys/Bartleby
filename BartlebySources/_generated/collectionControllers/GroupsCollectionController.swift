...
@objc(GroupsCollectionController) public class GroupsCollectionController : BaseObject,IterableCollectibleCollection{
...
// MARK: Identifiable
    ...
    required public init?(coder decoder: NSCoder) {
		...
		items=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Group.classForCoder()]), forKey: "items")! as! [Group]
		_startObservingAllItems()
    ...
    public func insertObject(item: Collectible, inItemsAtIndex index: Int) {
        if let item=item as? Group{
            ...
            if item.committed==false{
               CreateGroup.commit(item, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
            }
    ...
    public func removeObjectFromItemsAtIndex(index: Int) {
        if let item : Group = items[index] {
            ...
            DeleteGroup.commit(item.UID,fromDataSpace:self.spaceUID, observableBy: self.observableByUID)  
    ...
    override public func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if let group = object as? Group{
            UpdateGroup.commit(group, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
        }
        if let undoManager = self.undoManager{
