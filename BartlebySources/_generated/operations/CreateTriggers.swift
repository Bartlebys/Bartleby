...
@objc(CreateTriggers) public class CreateTriggers : BaseObject,JHTTPCommand{
    ...
    */
    static func commit(triggers:[Trigger], inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateTriggers(triggers,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:3401296095, caller: "CreateTriggers.commit")
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateTriggers.execute(self._triggers,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._triggers)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:603059392, caller: "CreateTriggers.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
...
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"CreateTriggers" ,forMethod:"POST", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 1106014024,
                        caller: "CreateTriggers.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of triggers",
                            comment: "creation of triggers failure description")
