...
import ObjectMapper
#endif
@objc(UpdateTriggers) public class UpdateTriggers : BaseObject,JHTTPCommand{
    ...
    required public init?(coder decoder: NSCoder) {
		...
		_triggers=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Trigger.classForCoder()]), forKey: "_triggers")! as! [Trigger]
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(_triggers,forKey:"_triggers")
    ...
    - parameter triggers: the triggers concerned the operation
    ...
    */
    init (_ triggers:[Trigger]=[Trigger](), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
    ...
    - parameter triggers: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(triggers:[Trigger], inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=UpdateTriggers(triggers,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:1783515425, caller: "UpdateTriggers.commit")
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            //if registry.upsert(self._triggers){                // Prepare the operation
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let _ = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                UpdateTriggers.execute(self._triggers,
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:790455321, caller: "UpdateTriggers.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(triggers:[Trigger],
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                for trigger in triggers{
                    let serializedInstance=Mapper<Trigger>().toJSON(trigger)
                    collection.append(serializedInstance)
                }
                parameters["triggers"]=collection
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"UpdateTriggers" ,forMethod:"PUT", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 457786802,
                        caller: "UpdateTriggers.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of triggers",
                            comment: "update of triggers failure description")
