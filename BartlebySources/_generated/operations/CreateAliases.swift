...
import ObjectMapper
#endif
@objc(CreateAliases) public class CreateAliases : BaseObject,JHTTPCommand{

    private var _aliases:[Alias] = [Alias]()
    ...
    required public convenience init(){
        self.init([Alias](), inDataSpace:Default.NO_UID,observableBy:Default.NOT_OBSERVABLE)
    }
    ...
    required public init?(coder decoder: NSCoder) {
		...
		_aliases=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Alias.classForCoder()]), forKey: "_aliases")! as! [Alias]
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(_aliases,forKey:"_aliases")
    ...
    - parameter aliases: the aliases concerned the operation
    ...
    */
    init (_ aliases:[Alias]=[Alias](), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
    ...
    - parameter aliases: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(aliases:[Alias], inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateAliases(aliases,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:838675221, caller: "CreateAliases.commit")
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            //if registry.upsert(self._aliases){                // Prepare the operation
                ...
                do{
                    ...
                    //Bartleby.bprint("\(ic.UID)->OPCOUNT=\(ic.items.count)",file:#file,function:#function,line:#line)
                }catch{
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateAliases.execute(self._aliases,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._aliases)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:1053831180, caller: "CreateAliases.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(aliases:[Alias],
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                for alias in aliases{
                    let serializedInstance=Mapper<Alias>().toJSON(alias)
                    collection.append(serializedInstance)
                }
                parameters["aliases"]=collection
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"CreateAliases" ,forMethod:"POST", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 983958117,
                        caller: "CreateAliases.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of aliases",
                            comment: "creation of aliases failure description")
