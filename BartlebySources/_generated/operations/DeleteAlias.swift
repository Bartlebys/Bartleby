...
@objc(DeleteAlias) public class DeleteAlias : BaseObject,JHTTPCommand{
    ...
    required public init?(coder decoder: NSCoder) {
		...
		_aliasId=String(decoder.decodeObjectOfClass(NSString.self, forKey: "_aliasId")! as NSString)
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(_aliasId,forKey:"_aliasId")
    ...
    - parameter aliasId: the aliasId concerned the operation
    ...
    */
    init (_ aliasId:String=String(), fromDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
    ...
    - parameter aliasId: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(aliasId:String, fromDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=DeleteAlias(aliasId,fromDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:2333687068, caller: "DeleteAlias.commit")
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            //if registry.deleteById(self._aliasId, fromCollectionWithName:"aliases"){                // Prepare the operation
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let _ = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                DeleteAlias.execute(self._aliasId,
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:1879390309, caller: "DeleteAlias.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(aliasId:String,
fromDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                parameters["aliasId"]=aliasId
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"DeleteAlias" ,forMethod:"DELETE", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 1136680663,
                        caller: "DeleteAlias.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("deleteById  of alias",
                            comment: "deleteById of alias failure description")
