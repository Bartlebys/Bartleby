...
import ObjectMapper
#endif
@objc(UpdateAlias) public class UpdateAlias : BaseObject,JHTTPCommand{
    ...
    required public init?(coder decoder: NSCoder) {
		...
		_alias=decoder.decodeObjectOfClass(Alias.self, forKey: "_alias")! 
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(_alias,forKey:"_alias")
    ...
    - parameter alias: the alias concerned the operation
    ...
    */
    init (_ alias:Alias=Alias(), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
        self._alias=alias
    ...
    - parameter alias: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(alias:Alias, inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=UpdateAlias(alias,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:1350064184, caller: "UpdateAlias.commit")
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            //if registry.upsert(self._alias){                // Prepare the operation
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let _ = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                UpdateAlias.execute(self._alias,
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:3159343630, caller: "UpdateAlias.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(alias:Alias,
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"UpdateAlias" ,forMethod:"PUT", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 2132335459,
                        caller: "UpdateAlias.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("update  of alias",
                            comment: "update of alias failure description")
