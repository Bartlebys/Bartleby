...
@objc(CreateAlias) public class CreateAlias : BaseObject,JHTTPCommand{
    ...
    */
    static func commit(alias:Alias, inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateAlias(alias,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:183745011, caller: "CreateAlias.commit")
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateAlias.execute(self._alias,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._alias)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:1014023571, caller: "CreateAlias.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
...
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                r.responseString{ response in
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of alias",
                            comment: "creation of alias failure description")
