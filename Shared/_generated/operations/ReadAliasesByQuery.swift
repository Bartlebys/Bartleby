...
@objc(ReadAliasesByQuery) public class ReadAliasesByQuery : BaseObject{
    ...
    public static func execute(fromDataSpace spaceUID:String,
				    ...
				    let pathURL=baseURL.URLByAppendingPathComponent("aliasesByQuery")
				    let dictionary:Dictionary<String, AnyObject>?=Mapper().toJSON(parameters)
				    let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"ReadAliasesByQuery" ,forMethod:"POST", and: pathURL)
				    ...
				    r.responseJSON{ response in
				        ...
				        let context = JHTTPResponse( code: 3073203273,
				            caller: "ReadAliasesByQuery.execute",
				           ...
				           let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
				            ...
				            if let statusCode=response?.statusCode {
				                if 200...299 ~= statusCode {
									if let instance = Mapper <Alias>().mapArray(result.value){					    
									    success(aliases: instance)
									  }else{
									   let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
									        context: context,
