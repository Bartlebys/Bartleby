...
import ObjectMapper
#endif
@objc(ReadTasksGroupsByQueryParameters) public class ReadTasksGroupsByQueryParameters : BaseObject {
	// 
    ...
    required public init?(coder decoder: NSCoder) {
		...
		query=decoder.decodeObjectOfClasses(NSSet(array: [NSDictionary.classForCoder(),NSString.classForCoder(),NSNumber.classForCoder(),NSObject.classForCoder(),NSSet.classForCoder()]), forKey: "query")as? Dictionary<String, AnyObject>
...
@objc(ReadTasksGroupsByQuery) public class ReadTasksGroupsByQuery : BaseObject{

    public static func execute(fromDataSpace spaceUID:String,
						parameters:ReadTasksGroupsByQueryParameters,
						sucessHandler success:(tasksGroups:[TasksGroup])->(),
						failureHandler failure:(context:JHTTPResponse)->()){
	
				    let baseURL=Bartleby.sharedInstance.getCollaborationURLForSpaceUID(spaceUID)
				    let pathURL=baseURL.URLByAppendingPathComponent("tasksGroupsByQuery")
				    let dictionary:Dictionary<String, AnyObject>?=Mapper().toJSON(parameters)
				    let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"ReadTasksGroupsByQuery" ,forMethod:"POST", and: pathURL)
				    ...
				    r.responseJSON{ response in
				        ...
				        let context = JHTTPResponse( code: 2176455555,
				            caller: "ReadTasksGroupsByQuery.execute",
				           ...
				           let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
				            ...
				            if let statusCode=response?.statusCode {
				                if 200...299 ~= statusCode {
									if let instance = Mapper <TasksGroup>().mapArray(result.value){					    
									    success(tasksGroups: instance)
									  }else{
									   let failureReaction =  Bartleby.Reaction.DispatchAdaptiveMessage(
									        context: context,
