...
import ObjectMapper
#endif
@objc(CreateTask) public class CreateTask : BaseObject,JHTTPCommand{
    ...
    required public init?(coder decoder: NSCoder) {
		...
		self._task=decoder.decodeObjectOfClass(Task.self, forKey: "_task")! 
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(self._task,forKey:"_task")
    ...
    - parameter task: the task concerned the operation
    ...
    */
    init (_ task:Task=Task(), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
        self._task=task
    ...
    - parameter task: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(task:Task, inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateTask(task,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:3881537104, caller: "CreateTask.commit")
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateTask.execute(self._task,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._task)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:1384718089, caller: "CreateTask.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(task:Task,
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                let pathURL=baseURL.URLByAppendingPathComponent("task")
                var parameters=Dictionary<String, AnyObject>()
                parameters["task"]=Mapper<Task>().toJSON(task)
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"CreateTask" ,forMethod:"POST", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 1019613663,
                        caller: "CreateTask.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of task",
                            comment: "creation of task failure description")
