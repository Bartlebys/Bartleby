...
import ObjectMapper
#endif
@objc(CreateTasks) public class CreateTasks : BaseObject,JHTTPCommand{
    ...
    required public convenience init(){
        self.init([Task](), inDataSpace:Default.NO_UID,observableBy:Default.NOT_OBSERVABLE)
    }
    ...
    required public init?(coder decoder: NSCoder) {
		...
		self._tasks=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Task.classForCoder()]), forKey: "_tasks")! as! [Task]
    ...
    override public func encodeWithCoder(coder: NSCoder) {
		...
		coder.encodeObject(self._tasks,forKey:"_tasks")
    ...
    - parameter tasks: the tasks concerned the operation
    ...
    */
    init (_ tasks:[Task]=[Task](), inDataSpace spaceUID:String,observableBy observationUID:String=Default.NOT_OBSERVABLE) {
        self._tasks=tasks
    ...
    - parameter tasks: the instance
    - parameter spaceUID:     the space UID
    - parameter oID:     the observable UID
    */
    static func commit(tasks:[Task], inDataSpace spaceUID:String,observableBy observationUID:String){
        let operationInstance=CreateTasks(tasks,inDataSpace:spaceUID,observableBy:observationUID)
        operationInstance.commit()
    }


    func commit(){
        let context=Context(code:4034852263, caller: "CreateTasks.commit")
    ...
    public func push(sucessHandler success:(context:HTTPResponse)->(),
        ...
        if let registry = Bartleby.sharedInstance.getRegistryByUID(self._spaceUID) {
            ...
            if  self._operation.status==Operation.Status.Pending ||
                ...
                CreateTasks.execute(self._tasks,
                    inDataSpace:self._spaceUID,
                    sucessHandler: { (context: JHTTPResponse) -> () in
                        registry.markAsDistributed(&self._tasks)
                ...
                // This registry is not available there is nothing to do.
                let context=Context(code:1169365701, caller: "CreateTasks.push")
                Bartleby.sharedInstance.dispatchAdaptiveMessage(context,
    ...
    static public func execute(tasks:[Task],
inDataSpace spaceUID:String,
            ...
            failureHandler failure:(context:JHTTPResponse)->()){
                ...
                for task in tasks{
                    let serializedInstance=Mapper<Task>().toJSON(task)
                    collection.append(serializedInstance)
                }
                parameters["tasks"]=collection
                let urlRequest=HTTPManager.mutableRequestWithToken(inDataSpace:spaceUID,withActionName:"CreateTasks" ,forMethod:"POST", and: pathURL)
                ...
                r.responseString{ response in
                    ...
                    let context = JHTTPResponse( code: 520639259,
                        caller: "CreateTasks.execute",
                    ...
                    if result.isFailure {
                        let m = NSLocalizedString("creation  of tasks",
                            comment: "creation of tasks failure description")
