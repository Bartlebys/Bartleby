...
@objc(TasksCollectionController) public class TasksCollectionController : BaseObject,IterableCollectibleCollection{
...
// MARK: Identifiable
    ...
    required public init?(coder decoder: NSCoder) {
		...
		items=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Task.classForCoder()]), forKey: "items")! as! [Task]
		_startObservingAllItems()
        ...
        if let item=item as? Task{
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("AddTask", comment: "AddTask undo action"))
                }
            }
            ...
            if item.committed==false{
               CreateTask.commit(item, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
            }
    ...
    public func removeObjectFromItemsAtIndex(index: Int) {
        if let item : Task = items[index] {
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("RemoveTask", comment: "Remove Task undo action"))
                }
            }
            ...
            DeleteTask.commit(item.UID,fromDataSpace:self.spaceUID, observableBy: self.observableByUID)  
    ...
    private func _startObserving(item: Task) {
        if _observedUIDS.indexOf(item.UID) == nil {
			...
			item.addObserver(self, forKeyPath: "onFailure", options: .Old, context: &KVOContext)
    ...
    private func _stopObserving(item: Task) {
        if let idx=_observedUIDS.indexOf(item.UID)  {
			...
			item.removeObserver(self, forKeyPath: "onFailure", context: &KVOContext)
    ...
    override public func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if let task = object as? Task{
            UpdateTask.commit(task, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
        }
        if let undoManager = self.undoManager{
