//
//  AliasesCollectionController.swift
//  Bartleby
//
...
// MARK: A  collection controller of "aliases"
...
@objc(AliasesCollectionController) public class AliasesCollectionController : BaseObject,IterableCollectibleCollection{
    ...
    public func generate() -> AnyGenerator<Alias> {
        var nextIndex = -1
    ...
    dynamic public var items:[Alias]=[Alias]()
...
// MARK: Identifiable
    ...
    required public init?(coder decoder: NSCoder) {
		...
		self.items=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Alias.classForCoder()]), forKey: "items")! as! [Alias]
		_startObservingAllItems()
        ...
        if let item=item as? Alias{
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("AddAlias", comment: "AddAlias undo action"))
                }
            }
            ...
            if let arrayController = self.arrayController{
                ...
                let sorted = arrayController.arrangedObjects as! [Alias]
            ...
            if item.committed==false{
               CreateAlias.commit(item, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
            }
    ...
    public func removeObjectFromItemsAtIndex(index: Int) {
        if let item : Alias = items[index] {
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("RemoveAlias", comment: "Remove Alias undo action"))
                }
            }
            ...
            DeleteAlias.commit(item.UID,fromDataSpace:self.spaceUID, observableBy: self.observableByUID)  
    ...
    private func _startObserving(item: Alias) {
    ...
    private func _stopObserving(item: Alias) {
        if self.enableKVO{
    ...
    override public func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if let alias = object as? Alias{
            UpdateAlias.commit(alias, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
        }
        if let undoManager = self.undoManager{
