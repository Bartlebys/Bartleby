...
@objc(PermissionsCollectionController) public class PermissionsCollectionController : BaseObject,IterableCollectibleCollection{
...
// MARK: Identifiable
    ...
    required public init?(coder decoder: NSCoder) {
		...
		items=decoder.decodeObjectOfClasses(NSSet(array: [NSArray.classForCoder(),Permission.classForCoder()]), forKey: "items")! as! [Permission]
		_startObservingAllItems()
        ...
        if let item=item as? Permission{
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("AddPermission", comment: "AddPermission undo action"))
                }
            }
            ...
            if item.committed==false{
               CreatePermission.commit(item, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
            }
    ...
    public func removeObjectFromItemsAtIndex(index: Int) {
        if let item : Permission = items[index] {
            ...
            if let undoManager: NSUndoManager = undoManager {
                ...
                if !undoManager.undoing {
                    undoManager.setActionName(NSLocalizedString("RemovePermission", comment: "Remove Permission undo action"))
                }
            }
            ...
            DeletePermission.commit(item.UID,fromDataSpace:self.spaceUID, observableBy: self.observableByUID)  
    ...
    private func _startObserving(item: Permission) {
        if _observedUIDS.indexOf(item.UID) == nil && self.enableKVO {
			...
			item.addObserver(self, forKeyPath: "rule", options: .Old, context: &KVOContext)
        }
    }
    ...
    override public func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if let permission = object as? Permission{
            UpdatePermission.commit(permission, inDataSpace:self.spaceUID, observableBy: self.observableByUID)
        }
        if let undoManager = self.undoManager{
